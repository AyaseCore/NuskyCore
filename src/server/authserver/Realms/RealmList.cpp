...
{
    NC_LOG_INFO("server.authserver", "Updating Realm List...");
    ...
    if (result)
    {
        do
        {
            ...
            if (init)
                NC_LOG_INFO("server.authserver", "Added realm \"%s\" at %s:%u.", name.c_str(), m_realms[name].ExternalAddress.get_host_addr(), port);
        }
