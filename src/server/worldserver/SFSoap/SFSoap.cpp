...
void SFSoapRunnable::run()
{
    ...
    {
        NC_LOG_ERROR("network.soap", "Couldn't bind to %s:%d", _host.c_str(), _port);
        exit(-1);
    }

    NC_LOG_INFO("network.soap", "Bound to http://%s:%d", _host.c_str(), _port);
    ...
    while (!World::IsStopped())
    {
        ...
        NC_LOG_DEBUG("network.soap", "Accepted connection from IP=%d.%d.%d.%d", (int)(soap.ip>>24)&0xFF, (int)(soap.ip>>16)&0xFF, (int)(soap.ip>>8)&0xFF, (int)soap.ip&0xFF);
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    {
        NC_LOG_INFO("network.soap", "Client didn't provide login information");
        return 401;
    }
    ...
    if (!accountId)
    {
        NC_LOG_INFO("network.soap", "Client used invalid username '%s'", soap->userid);
        return 401;
    }

    if (!AccountMgr::CheckPassword(accountId, soap->passwd))
    {
        NC_LOG_INFO("network.soap", "Invalid password for account '%s'", soap->userid);
        return 401;
    }

    if (AccountMgr::GetSecurity(accountId) < SEC_ADMINISTRATOR)
    {
        NC_LOG_INFO("network.soap", "%s's gmlevel is too low", soap->userid);
        return 403;
    }
    ...
    NC_LOG_INFO("network.soap", "Received command '%s'", command);
    ...
    if (acc)
        NC_LOG_ERROR("network.soap", "Error while acquiring lock, acc = %i, errno = %u", acc, errno);
