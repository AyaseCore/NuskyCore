...
int RASocket::open(void *)
{
    ...
    {
        NC_LOG_ERROR("server.worldserver", "RASocket::open: peer().get_remote_addr error is %s", ACE_OS::strerror(errno));
        return -1;
    }

    NC_LOG_INFO("commands.ra", "Incoming connection from %s", remoteAddress.get_host_addr());
...
{
    NC_LOG_INFO("commands.ra", "Closing connection");
    peer().close_reader();
    wait();
...
int RASocket::recv_line(std::string& out_line)
{
    ...
    {
        NC_LOG_DEBUG("commands.ra", "Recv error %s", ACE_OS::strerror(errno));
        return -1;
    }
...
int RASocket::process_command(const std::string& command)
{
    ...
    NC_LOG_INFO("commands.ra", "Received command: %s", command.c_str());
...
int RASocket::check_access_level(const std::string& user)
{
    ...
    if (!result)
    {
        NC_LOG_INFO("commands.ra", "User %s does not exist in database", user.c_str());
        return -1;
    }
    ...
    {
        NC_LOG_INFO("commands.ra", "User %s has no privilege to login", user.c_str());
        return -1;
    }
    else if (fields[2].GetInt32() != -1)
    {
        NC_LOG_INFO("commands.ra", "User %s has to be assigned on all realms (with RealmID = '-1')", user.c_str());
        return -1;
    }
...
int RASocket::check_password(const std::string& user, const std::string& pass)
{
    ...
    if (!result)
    {
        NC_LOG_INFO("commands.ra", "Wrong password for user: %s", user.c_str());
        return -1;
    }
...
int RASocket::authenticate()
{
    ...
    NC_LOG_INFO("commands.ra", "Login attempt for user: %s", user.c_str());
    ...
    NC_LOG_INFO("commands.ra", "User login: %s", user.c_str());
...
int RASocket::subnegotiate()
{
    ...
    if (n >= 1024)
    {
        NC_LOG_DEBUG("commands.ra", "RASocket::subnegotiate: allocated buffer 1024 bytes was too small for negotiation packet, size: %u", uint32(n));
        return -1;
    }
    ...
    for (uint8 i = 0; i < n; )
    {
        ...
        if (iac == 0xFF)   // "Interpret as Command" (IAC)
        {
            ...
            uint8 param = buf[++i];
            ss << uint32(param);
            NC_LOG_DEBUG("commands.ra", ss.str().c_str());
        }
        ++i;
    }
    #endif
...
void RASocket::zprint(void* callbackArg, const char * szText)
{
    ...
    {
        NC_LOG_DEBUG("commands.ra", "Failed to enqueue message, queue is full or closed. Error is %s", ACE_OS::strerror(errno));
        mb->release();
    }
}
...
void RASocket::commandFinished(void* callbackArg, bool /*success*/)
{
    ...
    if (socket->putq(mb->duplicate()) == -1)
        ...
        NC_LOG_DEBUG("commands.ra", "Failed to enqueue command end message. Error is %s", ACE_OS::strerror(errno));
