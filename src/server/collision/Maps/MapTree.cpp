...
namespace VMAP
{
    ...
    class AreaInfoCallback
    {
        public:
            ...
            {
#ifdef VMAP_DEBUG
                NC_LOG_DEBUG("maps", "AreaInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    class LocationInfoCallback
    {
        public:
            ...
            {
#ifdef VMAP_DEBUG
                NC_LOG_DEBUG("maps", "LocationInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    */
    ...
    bool StaticMapTree::InitMap(const std::string &fname, VMapManager2* vm)
    {
        ...
        ModelSpawn spawn;
#ifdef VMAP_DEBUG
        NC_LOG_DEBUG("maps", "StaticMapTree::InitMap() : map isTiled: %u", static_cast<uint32>(iIsTiled));
#endif
                ...
                if (result)
                {
                    ...
                    if (fread(&referencedVal, sizeof(uint32), 1, tf) == 1)
                    {
                        if (!iLoadedSpawns.count(referencedVal))
                        {
                            ...
                            {
                                NC_LOG_DEBUG("maps", "StaticMapTree::LoadMapTile() : invalid tree element (%u/%u)", referencedVal, iNTreeValues);
                                continue;
                            }
#endif
                        ...
                        else
                        {
                            ...
                            if (iTreeValues[referencedVal].ID != spawn.ID)
                                NC_LOG_DEBUG("maps", "StaticMapTree::LoadMapTile() : trying to load wrong spawn in node");
                            else if (iTreeValues[referencedVal].name != spawn.name)
                                NC_LOG_DEBUG("maps", "StaticMapTree::LoadMapTile() : name collision on GUID=%u", spawn.ID);
#endif
                        }
                    }
                    else
