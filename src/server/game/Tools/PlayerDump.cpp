        ...
        do
        {
            ...
            switch (type)
            {
                ...
                case DTT_CHARACTER:
                {
                    ...
                    {
                        NC_LOG_FATAL("misc", "PlayerDumpWriter::DumpTable - Trying to access non-existing or wrong positioned field (`deleteInfos_Account`) in `characters` table.");
                        return false;
                    }
...
DumpReturn PlayerDumpReader::LoadDump(std::string const& file, uint32 account, std::string name, uint32 guid)
{
    ...
    while (!feof(fin))
    {
        ...
        */
        ...
        if (tn.empty())
        {
            NC_LOG_ERROR("misc", "LoadPlayerDump: Can't extract table name from line: '%s'!", line.c_str());
            ROLLBACK(DUMP_FILE_BROKEN);
        }
        ...
        {
            NC_LOG_ERROR("misc", "LoadPlayerDump: Unknown table: '%s'!", tn.c_str());
            ROLLBACK(DUMP_FILE_BROKEN);
        }
        ...
        switch (type)
        {
            ...
            default:
                NC_LOG_ERROR("misc", "Unknown dump table type: %u", type);
                break;
        }
