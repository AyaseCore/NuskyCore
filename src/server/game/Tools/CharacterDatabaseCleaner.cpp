...
void CharacterDatabaseCleaner::CleanDatabase()
{
    ...
    NC_LOG_INFO("misc", "Cleaning character database...");
    ...
    NC_LOG_INFO("server.loading", ">> Cleaned character database in %u ms", GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDatabaseCleaner::CheckUnique(const char* column, const char* table, bool (*check)(uint32))
{
    ...
    if (!result)
    {
        NC_LOG_INFO("misc", "Table %s is empty.", table);
        return;
    }
