...
void LoadFromDB()
{
    ...
    if (result)
    {
        ...
        NC_LOG_INFO("server.loading", ">> Loaded %u known addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
    else
        NC_LOG_INFO("server.loading", ">> Loaded 0 known addons. DB table `addons` is empty!");
    ...
    if (result)
    {
        ...
        NC_LOG_INFO("server.loading", ">> Loaded %u banned addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
}
