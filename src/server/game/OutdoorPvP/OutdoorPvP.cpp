...
{
    NC_LOG_DEBUG("outdoorpvp", "Creating capture point %u", entry);

    // check info existence
    GameObjectTemplate const* goinfo = sObjectMgr->GetGameObjectTemplate(entry);
    if (!goinfo || goinfo->type != GAMEOBJECT_TYPE_CAPTURE_POINT)
    {
        NC_LOG_ERROR("outdoorpvp", "OutdoorPvP: GO %u is not capture point!", entry);
        return false;
    }
...
bool OPvPCapturePoint::DelCreature(uint32 type)
{
    if (!m_Creatures[type])
    {
        NC_LOG_DEBUG("outdoorpvp", "opvp creature type %u was already deleted", type);
        return false;
    }
    ...
    NC_LOG_DEBUG("outdoorpvp", "deleting opvp creature type %u", type);
...
void OutdoorPvP::HandlePlayerLeaveZone(Player* player, uint32 /*zone*/)
{
    ...
    NC_LOG_DEBUG("outdoorpvp", "Player %s left an outdoorpvp zone", player->GetName().c_str());
}
...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    Nusky::AnyPlayerInObjectRangeCheck checker(m_capturePoint, radius);
    Nusky::PlayerListSearcher<Nusky::AnyPlayerInObjectRangeCheck> searcher(m_capturePoint, players, checker);
    ...
    {
        //NC_LOG_ERROR("outdoorpvp", "%u->%u", m_OldState, m_State);
        if (oldTeam != m_team)
            ChangeTeam(oldTeam);
        ChangeState();
        return true;
