    ...
    else                                                    // it's custom channel
    {
        ...
        if (sWorld->getBoolConfig(CONFIG_PRESERVE_CUSTOM_CHANNELS))
        {
            ...
            if (result) //load
            {
                ...
                if (db_BannedList)
                {
                    ...
                    for (Tokenizer::const_iterator i = tokens.begin(); i != tokens.end(); ++i)
                    {
                        ...
                        if (banned_guid)
                        {
                            NC_LOG_DEBUG("chat.system", "Channel(%s) loaded bannedStore guid:" UI64FMTD "", name.c_str(), banned_guid);
            ...
            else // save
            {
                ...
                NC_LOG_DEBUG("chat.system", "Channel(%s) saved in database", name.c_str());
            }
...
void Channel::UpdateChannelInDB() const
{
    if (_IsSaved)
    {
        ...
        NC_LOG_DEBUG("chat.system", "Channel(%s) updated in database", _name.c_str());
    }
}
...
void Channel::CleanOldChannelsInDB()
{
    if (sWorld->getIntConfig(CONFIG_PRESERVE_CUSTOM_CHANNEL_DURATION) > 0)
    {
        ...
        NC_LOG_DEBUG("chat.system", "Cleaned out unused custom chat channels.");
    }
}
...
void Channel::List(Player const* player)
{
    ...
    NC_LOG_DEBUG("chat.system", "SMSG_CHANNEL_LIST %s Channel: %s",
