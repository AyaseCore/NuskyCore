...
{
    const char *format = GetNuskyString(entry);
    char str[1024];
    va_list ap;
    va_start(ap, entry);
...
const char *ChatHandler::GetNuskyString(int32 entry) const
{
    return m_session->GetNuskyString(entry);
}
...
{
    SendSysMessage(GetNuskyString(entry));
}
...
bool ChatHandler::SetDataForCommandInTable(std::vector<ChatCommand>& table, char const* text, uint32 permission, std::string const& help, std::string const& fullcommand)
{
    ...
    for (uint32 i = 0; i < table.size(); i++)
    {
        ...
        {
            NC_LOG_ERROR("sql.sql", "Table `command` have unexpected subcommand '%s' in command '%s', skip.", text, fullcommand.c_str());
            return false;
        }

        if (table[i].Permission != permission)
            NC_LOG_INFO("misc", "Table `command` overwrite for command '%s' default permission (%u) by %u", fullcommand.c_str(), table[i].Permission, permission);
    ...
    if (!cmd.empty())
    {
        if (&table == &getCommandTable())
            NC_LOG_ERROR("sql.sql", "Table `command` have not existed command '%s', skip.", cmd.c_str());
        else
            NC_LOG_ERROR("sql.sql", "Table `command` have not existed subcommand '%s' in command '%s', skip.", cmd.c_str(), fullcommand.c_str());
    }
...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
    Nusky::NearestGameObjectCheck check(*pl);
    Nusky::GameObjectLastSearcher<Nusky::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        CellCoord p(Nusky::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
        Cell cell(p);

        Nusky::GameObjectWithDbGUIDCheck go_check(*pl, lowguid);
        Nusky::GameObjectSearcher<Nusky::GameObjectWithDbGUIDCheck> checker(pl, obj, go_check);

        TypeContainerVisitor<Nusky::GameObjectSearcher<Nusky::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
...
const char *CliHandler::GetNuskyString(int32 entry) const
{
...
{
    return GetNuskyString(LANG_CONSOLE_COMMAND);
}
