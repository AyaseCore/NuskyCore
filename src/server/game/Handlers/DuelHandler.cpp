...
void WorldSession::HandleDuelResponseOpcode(WorldPacket& recvPacket)
{
    ...
    if (accepted)
    {
        ...
        //NC_LOG_DEBUG("network", "WORLD: Received CMSG_DUEL_ACCEPTED");
        NC_LOG_DEBUG("network", "Player 1 is: %u (%s)", player->GetGUIDLow(), player->GetName().c_str());
        NC_LOG_DEBUG("network", "Player 2 is: %u (%s)", plTarget->GetGUIDLow(), plTarget->GetName().c_str());
