...
void WorldSession::HandleDismissCritter(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_DISMISS_CRITTER for (guid: %u)", uint32(GUID_LOPART(guid)));

    Unit* pet = ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, guid);

    if (!pet)
    {
        NC_LOG_DEBUG("network", "Vanitypet (guid: %u) does not exist - player '%s' (guid: %u / account: %u) attempted to dismiss it (possibly lagged out)",
...
void WorldSession::HandlePetAction(WorldPacket& recvData) //  sub_68C8FD [5.4.8 18291]
{
    ...
    NC_LOG_DEBUG("network", "HandlePetAction: Pet (GUID: %u) - Flag: %u, SpellID: %u, Target: %u.", uint32(GUID_LOPART(PetGUID)), uint32(Flag), SpellID, uint32(GUID_LOPART(TargetGUID)));

    if (!pet)
    {
        NC_LOG_ERROR("network", "HandlePetAction: Pet (GUID: %u) doesn't exist for player %s (GUID: %u)", uint32(GUID_LOPART(PetGUID)), GetPlayer()->GetName().c_str(), GUID_LOPART(GetPlayer()->GetGUID()));
        return;
    }

    if (pet != GetPlayer()->GetFirstControlled())
    {
        NC_LOG_ERROR("network", "HandlePetAction: Pet (GUID: %u) does not belong to player %s (GUID: %u)", uint32(GUID_LOPART(PetGUID)), GetPlayer()->GetName().c_str(), GUID_LOPART(GetPlayer()->GetGUID()));
        return;
    }
...
void WorldSession::HandlePetStopAttack(WorldPacket &recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_PET_STOP_ATTACK for GUID " UI64FMTD "", (uint64)guid);

    Unit* pet = ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, guid);

    if (!pet)
    {
        NC_LOG_ERROR("network", "HandlePetStopAttack: Pet %u does not exist", uint32(GUID_LOPART(guid)));
        return;
    }

    if (pet != GetPlayer()->GetPet() && pet != GetPlayer()->GetCharm())
    {
        NC_LOG_ERROR("network", "HandlePetStopAttack: Pet GUID %u isn't a pet or charmed creature of player %s",
...
void WorldSession::HandlePetActionHelper(Unit* pet, uint64 guid1, uint32 spellid, uint16 flag, uint64 guid2, float x, float y, float z)
{
    ...
    if (!charmInfo)
    {
        NC_LOG_ERROR("network", "WorldSession::HandlePetAction(petGuid: " UI64FMTD ", tagGuid: " UI64FMTD ", spellId: %u, flag: %u): object (entry: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!",
    ...
    switch (flag)
    {
        case ACT_COMMAND:                                   //0x07
            switch (spellid)
            {
                ...
                case COMMAND_MOVE_TO:
                    ...
                    break;
                default:
                    NC_LOG_ERROR("network", "WORLD: unknown PET flag Action %i and spellid %i.", uint32(flag), spellid);
            }
            break;
        ...
        case ACT_ENABLED:                                   // 0xC1    spell
        {
            ...
            if (!spellInfo)
            {
                NC_LOG_ERROR("network", "WORLD: unknown PET spell id %i", spellid);
                return;
            }
...
{
    NC_LOG_INFO("network", "HandlePetNameQuery. CMSG_PET_NAME_QUERY");
...
bool WorldSession::CheckStableMaster(uint64 guid)
{
    ...
    if (guid == GetPlayer()->GetGUID())
    {
        ...
        {
            NC_LOG_DEBUG("network", "Player (GUID:%u) attempt open stable in cheating way.", GUID_LOPART(guid));
            return false;
        }
    }
    // stable master case
    else
    {
        ...
        {
            NC_LOG_DEBUG("network", "Stablemaster (GUID:%u) not found or you can't interact with him.", GUID_LOPART(guid));
            return false;
        }
    }
    return true;
...
{
    NC_LOG_INFO("network", "HandlePetSetAction. CMSG_PET_SET_ACTION");
    ...
    {
        NC_LOG_ERROR("network", "HandlePetSetAction: Unknown pet (GUID: %u) or pet owner (GUID: %u)", GUID_LOPART(petguid), _player->GetGUIDLow());
        return;
    }
    ...
    if (!charmInfo)
    {
        NC_LOG_ERROR("network", "WorldSession::HandlePetSetAction: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUIDLow(), pet->GetTypeId());
        return;
    }
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        NC_LOG_INFO("network", "Player %s has changed pet spell action. Position: %u, Spell: %u, State: 0x%X",
...
{
    NC_LOG_INFO("network", "HandlePetRename. CMSG_PET_RENAME");
...
void WorldSession::HandlePetAbandon(WorldPacket& recvData)
{
    ...
    NC_LOG_INFO("network", "HandlePetAbandon. CMSG_PET_ABANDON pet guid is %u", GUID_LOPART(guid));
...
{
    NC_LOG_INFO("network", "CMSG_PET_SPELL_AUTOCAST");
    ObjectGuid petGUID;
    uint32 spellId;
    bool state;
    ...
    {
        NC_LOG_ERROR("network", "HandlePetSpellAutocastOpcode.Pet %u isn't pet of player %s (GUID: %u).", uint32(GUID_LOPART(petGUID)), GetPlayer()->GetName().c_str(), GUID_LOPART(GetPlayer()->GetGUID()));
        return;
    }
    ...
    if (!charmInfo)
    {
        NC_LOG_ERROR("network", "WorldSession::HandlePetSpellAutocastOpcod: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUIDLow(), pet->GetTypeId());
        return;
    }
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_PET_CAST_SPELL");
    ...
    NC_LOG_DEBUG("network", "WORLD: CMSG_PET_CAST_SPELL, guid: " UI64FMTD ", castCount: %u, spellId %u, castFlags %u", guid, castCount, spellId, castFlags);
    ...
    {
        NC_LOG_ERROR("network", "HandlePetCastSpellOpcode: Pet %u isn't pet of player %s (GUID: %u).", uint32(GUID_LOPART(guid)), GetPlayer()->GetName().c_str(), GUID_LOPART(GetPlayer()->GetGUID()));
        return;
    }
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_PET_LEARN_TALENT");
}

void WorldSession::HandleLearnPreviewTalentsPet(WorldPacket& recvData)
{
    NC_LOG_DEBUG("network", "CMSG_LEARN_PREVIEW_TALENTS_PET");
}
