...
{
    NC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_BUY");
    ...
    NC_LOG_DEBUG("network", "Petitioner with GUID %u tried sell petition: name %s", GUID_LOPART(guidNPC), name.c_str());

    // prevent cheating
    Creature* creature = GetPlayer()->GetNPCIfCanInteractWith(guidNPC, UNIT_NPC_FLAG_PETITIONER);
    if (!creature)
    {
        NC_LOG_DEBUG("network", "WORLD: HandlePetitionBuyOpcode - Unit (GUID: %u) not found or you can't interact with him.", GUID_LOPART(guidNPC));
        return;
    }
    ...
    NC_LOG_DEBUG("network", "Invalid petition GUIDs: %s", ssInvalidPetitionGUIDs.str().c_str());
...
{
    NC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_SHOW_SIGNATURES");
    ...
    if (!result)
    {
        NC_LOG_DEBUG("entities.player.items", "Petition %u is not found for player %u %s", GUID_LOPART(petitionGuid), GetPlayer()->GetGUIDLow(), GetPlayer()->GetName().c_str());
        return;
    }
    ...
    NC_LOG_DEBUG("network", "CMSG_PETITION_SHOW_SIGNATURES petition entry: '%u'", petitionGuidLow);
...
{
    NC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_QUERY");   // ok

    uint32 guildguid;
    ObjectGuid petitionGuid;

    recvData >> guildguid;                                 // in Nusky always same as GUID_LOPART(petitionguid)
    ...
    NC_LOG_DEBUG("network", "CMSG_PETITION_QUERY Petition GUID %u Guild GUID %u", GUID_LOPART(petitionGuid), guildguid);
...
void WorldSession::SendPetitionQueryOpcode(uint64 petitionGuid)
{
    ...
    {
        NC_LOG_DEBUG("network", "CMSG_PETITION_QUERY failed for petition (GUID: %u)", GUID_LOPART(petitionGuid));
        return;
    }

    WorldPacket data(SMSG_PETITION_QUERY_RESPONSE, (4+8+name.size()+1+1+4*12+2+10));
    data << uint32(GUID_LOPART(petitionGuid));              // guild/team guid (in Nusky always same as GUID_LOPART(petition guid)
...
{
    NC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_RENAME");
    ...
    NC_LOG_DEBUG("network", "Petition (GUID: %u) renamed to '%s'", GUID_LOPART(petitionGuid), newName.c_str());
...
{
    NC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_SIGN");    // ok
    ...
    if (!result)
    {
        NC_LOG_ERROR("network", "Petition %u is not found for player %u %s", GUID_LOPART(petitionGuid), GetPlayer()->GetGUIDLow(), GetPlayer()->GetName().c_str());
        return;
    }
    ...
    NC_LOG_DEBUG("network", "PETITION SIGN: GUID %u by player: %s (GUID: %u Account: %u)", GUID_LOPART(petitionGuid), _player->GetName().c_str(), playerGuid, GetAccountId());
...
{
    NC_LOG_DEBUG("network", "Received opcode MSG_PETITION_DECLINE");  // ok
    ...
    NC_LOG_DEBUG("network", "Petition %u declined by %u", GUID_LOPART(petitionGuid), _player->GetGUIDLow());
...
{
    NC_LOG_DEBUG("network", "Received opcode CMSG_OFFER_PETITION");   // ok
    ...
    NC_LOG_DEBUG("network", "OFFER PETITION: type %u, GUID1 %u, to player id: %u", type, GUID_LOPART(petitionGuid), GUID_LOPART(playerGuid));
...
{
    NC_LOG_DEBUG("network", "Received opcode CMSG_TURN_IN_PETITION");
    ...
    NC_LOG_DEBUG("network", "Petition %u turned in by %u", GUID_LOPART(petitionGuid), _player->GetGUIDLow());
    ...
    {
        NC_LOG_ERROR("network", "Player %s (guid: %u) tried to turn in petition (guid: %u) that is not present in the database", _player->GetName().c_str(), _player->GetGUIDLow(), GUID_LOPART(petitionGuid));
        return;
    }
    ...
    // created
    NC_LOG_DEBUG("network", "TURN IN PETITION GUID %u", GUID_LOPART(petitionGuid));
...
{
    NC_LOG_DEBUG("network", "Received CMSG_PETITION_SHOWLIST");
    ...
    if (!unit)
    {
        NC_LOG_DEBUG("network", "WORLD: HandlePetitionerShowListOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
void WorldSession::SendPetitionShowList(uint64 guid)
{
    ...
    if (!creature)
    {
        NC_LOG_DEBUG("network", "WORLD: HandlePetitionShowListOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    SendPacket(&data);
    NC_LOG_DEBUG("network", "Sent SMSG_PETITION_SHOWLIST");
}
