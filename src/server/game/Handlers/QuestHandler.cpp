...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket& recvData)
{
    ...
    if (!questgiver)
    {
        NC_LOG_INFO("network", "Error in CMSG_QUEST_GIVER_STATUS_QUERY, called for non-existing questgiver (Typeid: %u GUID: %u)", GuidHigh2TypeId(GUID_HIPART(guid)), GUID_LOPART(guid));
        return;
    }
    ...
    case TYPEID_UNIT:
        {
            NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_STATUS_QUERY for npc, guid = %u", uint32(GUID_LOPART(guid)));
    ...
    case TYPEID_GAMEOBJECT:
        {
            NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_STATUS_QUERY for GameObject guid = %u", uint32(GUID_LOPART(guid)));
    ...
    default:
        NC_LOG_ERROR("network", "QuestGiver called for unexpected type %u", questgiver->GetTypeId());
        break;
    }
...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_HELLO npc = %u", GUID_LOPART(guid));

    Creature* creature = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_NONE);
    if (!creature)
    {
        NC_LOG_DEBUG("network", "WORLD: HandleQuestgiverHelloOpcode - Unit (GUID: %u) not found or you can't interact with him.",
            GUID_LOPART(guid));
        return;
    }
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_ACCEPT_QUEST npc = %u, quest = %u", uint32(GUID_LOPART(guid)), questId);
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_QUERY_QUEST npc = %u, quest = %u", uint32(GUID_LOPART(guid)), questId);
...
void WorldSession::HandleQuestQueryOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_QUERY quest = %u", questId);
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_CHOOSE_REWARD npc = %u, quest = %u, reward = %u", uint32(GUID_LOPART(guid)), questId, reward);
    ...
    if (!quest->GetRewardPackageItemId() > 1)
    {
        if (!quest->IsRewChoiceItemValid(reward))
        {
            NC_LOG_ERROR("network", "Error in CMSG_QUEST_GIVER_CHOOSE_REWARD: player %s (guid %d) tried to get invalid reward (%u) (possible packet-hacking detected)", _player->GetName().c_str(), _player->GetGUIDLow(), reward);
            return;
        }
    }
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
    {
        ...
        {
            NC_LOG_ERROR("network", "Error in QUEST_STATUS_COMPLETE: player %s (guid %u) tried to complete quest %u, but is not allowed to do so (possible packet-hacking or high latency)",
...
void WorldSession::HandleQuestgiverRequestRewardOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_REQUEST_REWARD npc = %u, quest = %u", uint32(GUID_LOPART(guid)), questId);
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_CANCEL");
...
void WorldSession::HandleQuestLogSwapQuest(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTLOG_SWAP_QUEST slot 1 = %u, slot 2 = %u", slot1, slot2);
...
void WorldSession::HandleQuestLogRemoveQuest(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTLOG_REMOVE_QUEST slot = %u", slot);
    ...
    if (slot < MAX_QUEST_LOG_SIZE)
    {
        if (uint32 questId = _player->GetQuestSlotQuestId(slot))
        {
            ...
            NC_LOG_INFO("network", "Player %u abandoned quest %u", _player->GetGUIDLow(), questId);
        }
...
void WorldSession::HandleQuestConfirmAccept(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_CONFIRM_ACCEPT questId = %u", questId);
...
void WorldSession::HandleQuestgiverCompleteQuest(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_COMPLETE_QUEST npc = %u, questId = %u", uint32(GUID_LOPART(playerGuid)), questId);
    ...
    if (!_player->CanSeeStartQuest(quest) && _player->GetQuestStatus(questId) == QUEST_STATUS_NONE)
    {
        NC_LOG_ERROR("network", "Possible hacking attempt: Player %s [guid: %u] tried to complete quest [entry: %u] without being in possession of the quest!",
            _player->GetName().c_str(), _player->GetGUIDLow(), questId);
        return;
    }
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_QUEST_AUTOLAUNCH");
}
...
void WorldSession::HandlePushQuestToParty(WorldPacket& recvPacket)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_PUSHQUESTTOPARTY questId = %u", questId);
...
void WorldSession::HandleQuestPushResult(WorldPacket& recvPacket)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received MSG_QUEST_PUSH_RESULT");
...
uint32 WorldSession::getDialogStatus(Player* player, Object* questgiver, uint32 defstatus)
{
    ...
    default:
        ...
        NC_LOG_ERROR("network", "Warning: GetDialogStatus called for unexpected type %u", questgiver->GetTypeId());
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_GIVER_STATUS_MULTIPLE_QUERY");
