...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
    ...
    {
        SendNotification(GetNuskyString(LANG_MAIL_SENDER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
    ...
    if (!receiverGuid)
    {
        NC_LOG_INFO("network", "Player %u is sending mail to %s (GUID: not existed!) with subject %s "
    ...
    NC_LOG_INFO("network", "Player %u is sending mail to %s (GUID: %u) with subject %s and body %s "
    ...
    {
        SendNotification(GetNuskyString(LANG_MAIL_RECEIVER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
            ...
            if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
            {
                ...
                else
                {
                    ...
                    if (!sObjectMgr->GetPlayerNameByGUID(sender_guid, sender_name))
                        sender_name = sObjectMgr->GetNuskyStringForDBCLocale(LANG_UNKNOWN);
                }
...
void WorldSession::HandleMailCreateTextItem(WorldPacket& recvData)
{
    ...
    NC_LOG_INFO("network", "HandleMailCreateTextItem mailid=%u", mailId);
