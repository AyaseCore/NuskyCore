...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket& recvData)
{
    ...
    {
        SendNotification(GetNuskyString(LANG_TICKET_REQ), sWorld->getIntConfig(CONFIG_TICKET_LEVEL_REQ));
        return;
    }
    ...
    if (!ticket || ticket->IsClosed())
    {
        ...
        if (count > 0)
        {
            ...
            if (count && decompressedSize && decompressedSize < 0xFFFF)
            {
                ...
                {
                    NC_LOG_ERROR("network", "CMSG_GM_TICKET_CREATE possibly corrupt. Uncompression failed.");
                    recvData.rfinish();
                    delete ticket;
                    return;
                }
