...
void WorldSession::HandleCharEnum(PreparedQueryResult result)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            NC_LOG_INFO("network", "Loading char guid %u from account %u.", guidLow, GetAccountId());
...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    if (!classEntry)
    {
        ...
        SendPacket(&data);
        NC_LOG_ERROR("network", "Class (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", class_, GetAccountId());
        return;
    }
    ...
    if (!raceEntry)
    {
        data << uint8(CHAR_CREATE_FAILED);
        SendPacket(&data);
        NC_LOG_ERROR("network", "Race (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", race_, GetAccountId());
        return;
    }
    /*
    ...
    NC_LOG_ERROR("network", "Expansion %u account:[%d] tried to Create character with expansion %u race (%u)", Expansion(), GetAccountId(), raceEntry->expansion, race_);
    SendPacket(&data);
    return;
    }
    ...
    NC_LOG_ERROR("network", "Expansion %u account:[%d] tried to Create character with expansion %u class (%u)", Expansion(), GetAccountId(), classEntry->expansion, class_);
    SendPacket(&data);
    return;
    }*/
    ...
    if (!normalizePlayerName(name))
    {
        ...
        SendPacket(&data);
        NC_LOG_ERROR("network", "Account:[%d] but tried to Create character with empty [name] ", GetAccountId());
        return;
    }
    ...
    case 3:
        {
            ...
            if (createInfo->Data.rpos() < createInfo->Data.wpos())
            {
                uint8 unk;
                createInfo->Data >> unk;
                NC_LOG_DEBUG("network", "Character creation %s (account %u) has unhandled tail data: [%u]", createInfo->Name.c_str(), GetAccountId(), unk);
            }
            ...
            NC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Create Character:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), createInfo->Name.c_str(), newChar.GetGUIDLow());
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "Character (Guid: %u) deleted", GUID_LOPART(guid));
    ...
    std::string IP_str = GetRemoteAddress();
    NC_LOG_INFO("entities.player.character", "Account: %d, IP: %s deleted character: %s, GUID: %u, Level: %u", accountId, IP_str.c_str(), name.c_str(), GUID_LOPART(guid), level);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket& recvData)
{
    ...
    {
        NC_LOG_ERROR("network", "Player tries to login again, AccountId = %d", GetAccountId());
        return;
    }
    ...
    NC_LOG_DEBUG("network", "WORLD: Recvd Player Logon Message");
    ...
    NC_LOG_DEBUG("network", "Character (Guid: %u) logging in", GUID_LOPART(playerGuid));

    if (!IsLegitCharacterForAccount(GUID_LOPART(playerGuid)))
    {
        NC_LOG_ERROR("network", "Account (%u) can't login with that character (%u).", GetAccountId(), GUID_LOPART(playerGuid));
        KickPlayer();
        return;
    }
...
{
    NC_LOG_INFO("general", "WORLD: Recvd CMSG_LOADING_SCREEN_NOTIFY");
    uint32 mapID;
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    // Send MOTD
    {
        ...
        SendPacket(&data);
        NC_LOG_DEBUG("network", "WORLD: Sent motd (SMSG_MOTD)");
    ...
    //NC_LOG_DEBUG("Player %s added to Map.", pCurrChar->GetName().c_str());
    ...
    if (pCurrChar->GetGuildId() != 0)
    {
        ...
        else
        {
            ...
            NC_LOG_ERROR("general", "Player %s (GUID: %u) marked as member of not existing guild (id: %u), removing guild membership for player.", pCurrChar->GetName().c_str(), pCurrChar->GetGUIDLow(), pCurrChar->GetGuildId());
            pCurrChar->SetInGuild(0);
        }
    }
    ...
    std::string IP_str = GetRemoteAddress();
    NC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Login Character:[%s] (GUID: %u) Level: %d",
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_LFG_BONUS_FACTION_ID");
    uint32 FactionID = 0;
    recvData >> FactionID;
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_ATWAR");
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_NOTATWAR");
...
{
    NC_LOG_ERROR("network", "WORLD SESSION: HandleSetFactionCheat, not expected call, please report.");
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_WATCHED_FACTION");
    uint32 fact;
    recvData >> fact;
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_INACTIVE");
    uint32 FactionIndex;
    uint8 Status;
...
{
    NC_LOG_DEBUG("network", "CMSG_SHOWING_HELM for %s", _player->GetName().c_str());
...
{
    NC_LOG_DEBUG("network", "CMSG_SHOWING_CLOAK for %s", _player->GetName().c_str());
    recvData.read_skip<uint8>(); // unknown, bool?
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(PreparedQueryResult result, std::string const& newName)
{
    ...
    NC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Character:[%s] (guid:%u) Changed name to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), guidLow, newName.c_str());
...
{
    NC_LOG_DEBUG("network", "CMSG_ALTER_APPEARANCE");
...
void WorldSession::HandleRemoveGlyph(WorldPacket& recvData)
{
    ...
    {
        NC_LOG_DEBUG("network", "Client sent wrong glyph slot number in opcode CMSG_REMOVE_GLYPH %u", slot);
        return;
    }
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
    ...
    {
        NC_LOG_ERROR("network", "Account %u, IP: %s tried to customise character %u, but it does not belong to their account!",
    ...
    if (result)
    {
        ...
        NC_LOG_INFO("entities.player.character", "Account: %d (IP: %s), Character[%s] (guid:%u) Customized to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldname.c_str(), GUID_LOPART(guid), newName.c_str());
    }
...
{
    NC_LOG_DEBUG("network", "CMSG_EQUIPMENT_SET_SAVE");
...
{
    NC_LOG_DEBUG("network", "CMSG_EQUIPMENT_SET_DELETE");
...
{
    NC_LOG_DEBUG("network", "CMSG_EQUIPMENT_SET_USE");
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    if (!IsLegitCharacterForAccount(GUID_LOPART(guid)))
    {
        NC_LOG_ERROR("network", "Account %u, IP: %s tried to factionchange character %u, but it does not belong to their account!",
            GetAccountId(), GetRemoteAddress().c_str(), GUID_LOPART(guid));
        KickPlayer();
        return;
    }
    ...
    std::string IP_str = GetRemoteAddress();
    NC_LOG_DEBUG("entities.player", "%s (IP: %s) changed race from %u to %u", GetPlayerInfo().c_str(), IP_str.c_str(), oldRace, race);
...
void WorldSession::HandleRandomizeCharNameOpcode(WorldPacket& recvData)
{
    ...
    {
        NC_LOG_ERROR("general", "Invalid race (%u) sent by accountId: %u", race, GetAccountId());
        return;
    }

    if (!Player::IsValidGender(gender))
    {
        NC_LOG_ERROR("general", "Invalid gender (%u) sent by accountId: %u", gender, GetAccountId());
        return;
    }
