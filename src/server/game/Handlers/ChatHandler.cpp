...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    switch (recvData.GetOpcode())
    {
        ...
        */
        default:
            NC_LOG_ERROR("network", "HandleMessagechatOpcode : Unknown chat opcode (%u)", recvData.GetOpcode());
            recvData.hexlike();
            return;
    }
    ...
    {
        NC_LOG_ERROR("network", "CHAT: Wrong message type received: %u", type);
        recvData.rfinish();
        return;
    }
    ...
    //NC_LOG_DEBUG("misc", "CHAT: packet received. type %u, lang %u", type, lang);
    ...
    if (type != CHAT_MSG_EMOTE && type != CHAT_MSG_AFK && type != CHAT_MSG_DND)
    {
        ...
        if (lang == LANG_ADDON)
        {
            ...
            switch (type)
            {
                ...
                case CHAT_MSG_WHISPER:
                    ...
                    break;
                default:
                    NC_LOG_ERROR("network", "Player %s (GUID: %u) sent a chatmessage with an invalid language/message type combination",
        ...
        else
        {
            ...
            if (!sender->CanSpeak())
            {
                ...
                SendNotification(GetNuskyString(LANG_WAIT_BEFORE_SPEAKING), timeStr.c_str());
    ...
    {
        SendNotification(GetNuskyString(LANG_GM_SILENCE), sender->GetName().c_str());
        recvData.rfinish();
        return;
    }
    ...
    if (!ignoreChecks)
    {
        ...
        if (lang != LANG_ADDON)
        {
            ...
            {
                NC_LOG_ERROR("network", "Player %s (GUID: %u) sent a chatmessage with an invalid link: %s", GetPlayer()->GetName().c_str(),
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            ...
            {
                SendNotification(GetNuskyString(LANG_SAY_REQ), sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            {
                SendNotification(GetNuskyString(LANG_WHISPER_REQ), sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
                return;
            }
            ...
            {
                SendNotification(GetNuskyString(LANG_GM_SILENCE), GetPlayer()->GetName().c_str());
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            if (!HasPermission(rbac::RBAC_PERM_SKIP_CHECK_CHAT_CHANNEL_REQ))
            {
                ...
                {
                    SendNotification(GetNuskyString(LANG_CHANNEL_REQ), sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
                    return;
                }
            }
        ...
        case CHAT_MSG_AFK:
        {
            if (!_player->IsInCombat())
            {
                ...
                {
                    _player->autoReplyMsg = msg.empty() ? GetNuskyString(LANG_PLAYER_AFK_DEFAULT) : msg;
        ...
        case CHAT_MSG_DND:
        {
            ...
            {
                _player->autoReplyMsg = msg.empty() ? GetNuskyString(LANG_PLAYER_DND_DEFAULT) : msg;
        ...
        default:
            NC_LOG_ERROR("network", "CHAT: unknown message type %u, lang: %u", type, lang);
            break;
    }
}
...
void WorldSession::HandleAddonMessagechatOpcode(WorldPacket& recvData)
{
    ...
    switch (recvData.GetOpcode())
    {
        ...
        case CMSG_MESSAGECHAT_ADDON_WHISPER:
            ...
            break;
        default:
            NC_LOG_ERROR("network", "HandleAddonMessagechatOpcode: Unknown addon chat opcode (%u)", recvData.GetOpcode());
            recvData.hexlike();
            return;
    }
    ...
    switch (type)
    {
        ...
        default:
        {
            NC_LOG_ERROR("misc", "HandleAddonMessagechatOpcode: unknown addon message type %u", type);
            break;
        }
    }
}
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket& recvData)
{
    ...
    CellCoord p = Nusky::ComputeCellCoord(GetPlayer()->GetPositionX(), GetPlayer()->GetPositionY());

    Cell cell(p);
    cell.SetNoCreate();

    Nusky::EmoteChatBuilder emote_builder(*GetPlayer(), text_emote, emoteNum, unit);
    Nusky::LocalizedPacketDo<Nusky::EmoteChatBuilder > emote_do(emote_builder);
    Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::EmoteChatBuilder > > emote_worker(GetPlayer(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), emote_do);
    TypeContainerVisitor<Nusky::PlayerDistWorker<Nusky::LocalizedPacketDo<Nusky::EmoteChatBuilder> >, WorldTypeMapContainer> message(emote_worker);
...
void WorldSession::HandleChatIgnoredOpcode(WorldPacket& recvData)
{
    ObjectGuid guid;
    uint8 unk;
    //NC_LOG_DEBUG("network", "WORLD: Received CMSG_CHAT_IGNORED");
...
{
    NC_LOG_DEBUG("network", "Opcode %u", recvPacket.GetOpcode());
}
