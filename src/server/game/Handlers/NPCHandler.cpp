...
void WorldSession::HandleBankerActivateOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_BANKER_ACTIVATE");
    ...
    if (!unit)
    {
        NC_LOG_DEBUG("network", "WORLD: HandleBankerActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
{
    std::string str = GetNuskyString(LANG_NPC_TAINER_HELLO);
    SendTrainerList(guid, str);
}

void WorldSession::SendTrainerList(uint64 guid, const std::string& strTitle)
{
    NC_LOG_DEBUG("network", "WORLD: SendTrainerList");

    Creature* unit = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_TRAINER);
    if (!unit)
    {
        NC_LOG_DEBUG("network", "WORLD: SendTrainerList - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    if (!ci)
    {
        NC_LOG_DEBUG("network", "WORLD: SendTrainerList - (GUID: %u) NO CREATUREINFO!", GUID_LOPART(guid));
        return;
    }
    ...
    if (!trainer_spells)
    {
        NC_LOG_DEBUG("network", "WORLD: SendTrainerList - Training spells not found for creature (GUID: %u Entry: %u)",
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_TRAINER_BUY_SPELL NpcGUID=%u, learn spell id is: %u", uint32(GUID_LOPART(guid)), spellId);

    Creature* unit = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_TRAINER);
    if (!unit)
    {
        NC_LOG_DEBUG("network", "WORLD: HandleTrainerBuySpellOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_GOSSIP_HELLO");
    ...
    if (!unit)
    {
        NC_LOG_DEBUG("network", "WORLD: HandleGossipHelloOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
/*void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket& recvData)
{
    ...
    {
        NC_LOG_DEBUG("network", "reading string");
        recvData >> code;
        NC_LOG_DEBUG("network", "string read: %s", code.c_str());
    }
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_SPIRIT_HEALER_ACTIVATE");
    ...
    if (!unit)
    {
        NC_LOG_DEBUG("network", "WORLD: HandleSpiritHealerActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(UnitGUID)));
        return;
    }
...
void WorldSession::HandleBinderActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit)
    {
        NC_LOG_DEBUG("network", "WORLD: HandleBinderActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(npcGuid)));
        return;
    }
...
{
    NC_LOG_DEBUG("network", "WORLD: Recv MSG_LIST_STABLED_PETS");
    uint64 npcGUID;
...
void WorldSession::SendStablePetCallback(PreparedQueryResult result, uint64 guid)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Recv MSG_LIST_STABLED_PETS Send.");
...
{
    NC_LOG_DEBUG("network", "WORLD: Recv CMSG_STABLE_PET");
    uint64 npcGUID;
...
{
    NC_LOG_DEBUG("network", "WORLD: Recv CMSG_UNSTABLE_PET.");
    uint64 npcGUID;
    uint32 petnumber;
...
{
    NC_LOG_DEBUG("network", "WORLD: Recv CMSG_BUY_STABLE_SLOT.");
    uint64 npcGUID;
...
{
    NC_LOG_DEBUG("network", "HandleStableRevivePet: Not implemented");
}

void WorldSession::HandleStableSwapPet(WorldPacket& recvData)
{
    NC_LOG_DEBUG("network", "WORLD: Recv CMSG_STABLE_SWAP_PET.");
    uint64 npcGUID;
    uint32 petId;
...
{
    NC_LOG_DEBUG("network", "WORLD: CMSG_REPAIR_ITEM");
    ...
    if (!unit)
    {
        NC_LOG_DEBUG("network", "WORLD: HandleRepairItemOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(npcGuid)));
        return;
    }
    ...
    if (itemGuid)
    {
        NC_LOG_DEBUG("network", "ITEM: Repair item, itemGUID = %u, npcGUID = %u", GUID_LOPART(itemGuid), GUID_LOPART(npcGuid));
    ...
    {
        NC_LOG_DEBUG("network", "ITEM: Repair all items, npcGUID = %u", GUID_LOPART(npcGuid));
