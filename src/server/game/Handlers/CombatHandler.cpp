...
void WorldSession::HandleAttackSwingOpcode(WorldPacket& recvData)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Recvd CMSG_ATTACKSWING Message guidlow:%u guidhigh:%u", GUID_LOPART(guid), GUID_HIPART(guid));
...
void WorldSession::HandleSetSheathedOpcode(WorldPacket& recvData)
{
    ...
    //NC_LOG_DEBUG("network", "WORLD: Recvd CMSG_SETSHEATHED Message guidlow:%u value1:%u", GetPlayer()->GetGUIDLow(), sheathed);

    if (hasData)
    {
        if (sheathed >= MAX_SHEATH_STATE)
        {
            NC_LOG_ERROR("network", "Unknown sheath state %u ??", sheathed);
            return;
        }
