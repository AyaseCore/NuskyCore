...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_TAXI_NODE_STATUS_QUERY");
...
void WorldSession::SendTaxiStatus(uint64 guid)
{
    ...
    if (!unit)
    {
        NC_LOG_DEBUG("network", "WorldSession::SendTaxiStatus - Unit (GUID: %u) not found.", uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    NC_LOG_DEBUG("network", "WORLD: current location %u ", curloc);
    ...
    NC_LOG_DEBUG("network", "WORLD: Sent SMSG_TAXI_NODE_STATUS");
}

void WorldSession::HandleTaxiQueryAvailableNodes(WorldPacket& recvData)
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_TAXI_QUERY_AVAILABLE_NODES");
    ...
    if (!unit)
    {
        NC_LOG_DEBUG("network", "WORLD: HandleTaxiQueryAvailableNodes - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
void WorldSession::SendTaxiMenu(Creature* unit)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: CMSG_TAXI_NODE_STATUS_QUERY %u ", curloc);
    ...
    NC_LOG_DEBUG("network", "WORLD: Sent SMSG_SHOW_TAXI_NODES");
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_ACTIVATE_TAXI_EXPRESS");
    ...
    if (!npc)
    {
        NC_LOG_DEBUG("network", "WORLD: HandleActivateTaxiExpressOpcode - Unit (GUID: %u) not found or you can't interact with it.", uint32(GUID_LOPART(guid)));
        return;
    }

    NC_LOG_DEBUG("network", "WORLD: Received CMSG_ACTIVATE_TAXI_EXPRESS from %d to %d", nodes.front(), nodes.back());
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_MOVE_SPLINE_DONE");
    ...
    if (destinationnode > 0)                              // if more destinations to go
    {
        ...
        NC_LOG_DEBUG("network", "WORLD: Taxi has to go from %u to %u", sourcenode, destinationnode);
...
{
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_ACTIVATE_TAXI");
    ...
    NC_LOG_DEBUG("network", "WORLD: Received CMSG_ACTIVATE_TAXI from %d to %d", nodes[0], nodes[1]);
    Creature* npc = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_FLIGHTMASTER);
    if (!npc)
    {
        NC_LOG_DEBUG("network", "WORLD: HandleActivateTaxiOpcode - Unit (GUID: %u) not found or you can't interact with it.", uint32(GUID_LOPART(guid)));
        return;
    }
...
void WorldSession::SendActivateTaxiReply(ActivateTaxiReply reply)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Sent SMSG_ACTIVATE_TAXI_REPLY");
}
