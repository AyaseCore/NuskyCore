...
#ifndef Nusky_CELLIMPL_H
#define Nusky_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = Nusky::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = Nusky::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = Nusky::ComputeCellCoord(x + radius, y + radius).normalize();
