...
uint32 AuctionHouseMgr::GetAuctionDeposit(AuctionHouseEntry const* entry, uint32 time, Item* pItem, uint32 count)
{
    ...
    NC_LOG_DEBUG("auctionHouse", "MSV:        %u", MSV);
    NC_LOG_DEBUG("auctionHouse", "Items:      %u", count);
    NC_LOG_DEBUG("auctionHouse", "Multiplier: %f", multiplier);
    NC_LOG_DEBUG("auctionHouse", "Deposit:    %u", deposit);
...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry* auction, SQLTransaction& trans)
{
    ...
    else
    {
        ...
        if (logGmTrade && !sObjectMgr->GetPlayerNameByGUID(bidderGuid, bidderName))
            bidderName = sObjectMgr->GetNuskyStringForDBCLocale(LANG_UNKNOWN);
    }
    ...
    if (logGmTrade)
    {
        ...
        if (!sObjectMgr->GetPlayerNameByGUID(auction->owner, ownerName))
            ownerName = sObjectMgr->GetNuskyStringForDBCLocale(LANG_UNKNOWN);
...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    if (!result)
    {
        NC_LOG_INFO("server.loading", ">> Loaded 0 auction items. DB table `auctionhouse` or `item_instance` is empty!");
    ...
    do
    {
        ...
        if (!proto)
        {
            NC_LOG_ERROR("misc", "AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: %u id: #%u) in auction, skipped.", item_guid, itemEntry);
            continue;
        }
    ...
    NC_LOG_INFO("server.loading", ">> Loaded %u auction items in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    if (!result)
    {
        NC_LOG_INFO("server.loading", ">> Loaded 0 auctions. DB table `auctionhouse` is empty.");
    ...
    NC_LOG_INFO("server.loading", ">> Loaded %u auctions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data) const
{
    ...
    if (!item)
    {
        NC_LOG_ERROR("misc", "AuctionEntry::BuildAuctionInfo: Auction %u has a non-existent item: %u", Id, itemGUIDLow);
        return false;
    }
    data << uint32(Id);
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    if (!auctioneerData)
    {
        NC_LOG_ERROR("misc", "Auction %u has not a existing auctioneer (GUID : %u)", Id, auctioneer);
        return false;
    }
    ...
    if (!auctioneerInfo)
    {
        NC_LOG_ERROR("misc", "Auction %u has not a existing auctioneer (GUID : %u Entry: %u)", Id, auctioneer, auctioneerData->id);
        return false;
    }
    ...
    {
        NC_LOG_ERROR("misc", "Auction %u has auctioneer (GUID : %u Entry: %u) with wrong faction %u", Id, auctioneer, auctioneerData->id, factionTemplateId);
        return false;
    }
    ...
    {
        NC_LOG_ERROR("misc", "Auction %u has not a existing item : %u", Id, itemGUIDLow);
        return false;
    }
    return true;
}
...
void AuctionHouseMgr::DeleteExpiredAuctionsAtStartup()
{
    ...
    if (!expAuctions)
    {
        NC_LOG_INFO("server.loading", ">> No expired auctions to delete");
    ...
    NC_LOG_INFO("server.loading", ">> Deleted %u expired auctions in %u ms", expirecount, GetMSTimeDiffToNow(oldMSTime));
...
bool AuctionEntry::LoadFromFieldList(Field* fields)
{
    ...
    if (!auctioneerData)
    {
        NC_LOG_ERROR("misc", "AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer (GUID : %u)", Id, auctioneer);
        return false;
    }
    ...
    if (!auctioneerInfo)
    {
        NC_LOG_ERROR("misc", "AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer (GUID : %u Entry: %u)", Id, auctioneer, auctioneerData->id);
        return false;
    }
    ...
    if (!auctionHouseEntry)
    {
        NC_LOG_ERROR("misc", "AuctionEntry::LoadFromFieldList() - Auction %u has auctioneer (GUID : %u Entry: %u) with wrong faction %u", Id, auctioneer, auctioneerData->id, factionTemplateId);
        return false;
    }
