...
bool Transport::Create(uint32 guidlow, uint32 entry, uint32 mapid, float x, float y, float z, float ang, uint32 animprogress)
{
    ...
    if (!IsPositionValid())
    {
        NC_LOG_ERROR("entities.transport", "Transport (GUID: %u) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
            guidlow, x, y);
        return false;
    }
    ...
    if (!goinfo)
    {
        NC_LOG_ERROR("sql.sql", "Transport not created: entry in `gameobject_template` not found, guidlow: %u map: %u  (X: %f Y: %f Z: %f) ang: %f", guidlow, mapid, x, y, z, ang);
        return false;
    }
    ...
    if (!tInfo)
    {
        NC_LOG_ERROR("sql.sql", "Transport %u (name: %s) will not be created, missing `transport_template` entry.", entry, goinfo->name.c_str());
        return false;
    }
...
void Transport::Update(uint32 diff)
{
    ...
    if (AI())
        ...
        NC_LOG_ERROR("entities.transport", "Could not initialize GameObjectAI for Transport");
    ...
    for (;;)
    {
        ...
        NC_LOG_DEBUG("entities.transport", "Transport %u (%s) moved to node %u %u %f %f %f", GetEntry(), GetName().c_str(), _currentFrame->Node->index, _currentFrame->Node->mapid, _currentFrame->Node->x, _currentFrame->Node->y, _currentFrame->Node->z);
...
void Transport::AddPassenger(WorldObject* passenger)
{
    ...
    {
        NC_LOG_DEBUG("entities.transport", "Object %s boarded transport %s.", passenger->GetName().c_str(), GetName().c_str());
...
void Transport::RemovePassenger(WorldObject* passenger)
{
    ...
    {
        NC_LOG_DEBUG("entities.transport", "Object %s removed from transport %s.", passenger->GetName().c_str(), GetName().c_str());
...
Creature* Transport::CreateNPCPassenger(uint32 guid, CreatureData const* data)
{
    ...
    {
        NC_LOG_ERROR("entities.transport", "Creature (guidlow %d, entry %d) not created. Suggested coordinates aren't valid (X: %f Y: %f)",
...
GameObject* Transport::CreateGOPassenger(uint32 guid, GameObjectData const* data)
{
    ...
    {
        NC_LOG_ERROR("entities.transport", "GameObject (guidlow %d, entry %d) not created. Suggested coordinates aren't valid (X: %f Y: %f)", go->GetGUIDLow(), go->GetEntry(), go->GetPositionX(), go->GetPositionY());
        delete go;
        return NULL;
    }
...
void Transport::DoEventIfAny(KeyFrame const& node, bool departure)
{
    ...
    {
        NC_LOG_DEBUG("maps.script", "Taxi %s event %u of node %u of %s path", departure ? "departure" : "arrival", eventid, node.Node->index, GetName().c_str());
