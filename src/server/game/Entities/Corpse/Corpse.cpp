...
bool Corpse::Create(uint32 guidlow, Player* owner)
{
    ...
    if (!IsPositionValid())
    {
        NC_LOG_ERROR("entities.player", "Corpse (guidlow %d, owner %s) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
    ...
    _gridCoord = Nusky::ComputeGridCoord(GetPositionX(), GetPositionY());
...
void Corpse::DeleteBonesFromWorld()
{
    ...
    if (!corpse)
    {
        NC_LOG_ERROR("entities.player", "Bones %u not found in world.", GetGUIDLow());
        return;
    }
...
bool Corpse::LoadCorpseFromDB(uint32 guid, Field* fields)
{
    ...
    if (!IsPositionValid())
    {
        NC_LOG_ERROR("entities.player", "Corpse (guid: %u, owner: %u) is not created, given coordinates are not valid (X: %f, Y: %f, Z: %f)",
