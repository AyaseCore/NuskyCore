...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current)
{
    ...
    if (!IsPositionValid())
    {
        NC_LOG_ERROR("entities.pet", "Pet (guidlow %d, entry %d) not loaded. Suggested coordinates isn't valid (X: %f Y: %f)",
    ...
    switch (getPetType())
    {
        ...
        default:
            if (!IsPetGhoul())
                NC_LOG_ERROR("entities.pet", "Pet have incorrect type (%u) for pet loading.", getPetType());
            break;
    }
    ...
    */
    ...
    NC_LOG_DEBUG("entities.pet", "New Pet has guid %u", GetGUIDLow());
    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            if (isControlled())
            {
                ...
                {
                    NC_LOG_ERROR("entities.pet", "Pet %u is not pet of owner %s, removed", GetEntry(), GetOwner()->GetName().c_str());
...
bool Pet::CreateBaseAtCreature(Creature* creature)
{
    ...
    if (!IsPositionValid())
    {
        NC_LOG_ERROR("entities.pet", "Pet (guidlow %d, entry %d) not created base at creature. Suggested coordinates isn't valid (X: %f Y: %f)",
            GetGUIDLow(), GetEntry(), GetPositionX(), GetPositionY());
        return false;
    }
    ...
    if (!cinfo)
    {
        NC_LOG_ERROR("entities.pet", "CreateBaseAtCreature() failed, creatureInfo is missing!");
        return false;
    }
...
{
    NC_LOG_DEBUG("entities.pet", "Pet::CreateBaseForTamed");
    uint32 guid=sObjectMgr->GenerateLowGuid(HIGHGUID_PET);
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    if (IsPet() && GetOwner()->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            NC_LOG_ERROR("entities.pet", "Unknown type pet %u is summoned by player class %u",
...
void Pet::_LoadSpellCooldowns()
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                NC_LOG_ERROR("entities.pet", "Pet %u have unknown spell %u in `pet_spell_cooldown`, skipping.", m_charmInfo->GetPetNumber(), spell_id);
                continue;
            }
            ...
            NC_LOG_DEBUG("entities.pet", "Pet (Number: %u) spell %u cooldown loaded (%u secs).", m_charmInfo->GetPetNumber(), spell_id, uint32(db_time-curTime));
        }
...
{
    NC_LOG_DEBUG("entities.pet", "Loading auras for pet %u", GetGUIDLow());
    ...
    if (result)
    {
        do
        {
            ...
            if (!spellInfo)
            {
                NC_LOG_ERROR("entities.pet", "Unknown aura (spellid %u), ignore.", spellid);
                continue;
            }
            ...
            if (Aura* aura = Aura::TryCreate(spellInfo, effMask, this, NULL, &baseDamage[0], NULL, caster_guid))
            {
                ...
                NC_LOG_INFO("entities.pet", "Added aura spellid %u, effectmask %u", spellInfo->Id, effMask);
            }
        }
        while (result->NextRow());
    }
...
bool Pet::addSpell(uint32 spellId, ActiveStates active /*= ACT_DECIDE*/, PetSpellState state /*= PETSPELL_NEW*/, PetSpellType type /*= PETSPELL_NORMAL*/)
{
    ...
    if (!spellInfo)
    {
        ...
        {
            NC_LOG_ERROR("entities.pet", "Pet::addSpell: Non-existed in SpellStore spell #%u request, deleting for all pets in `pet_spell`.", spellId);
            ...
            NC_LOG_ERROR("entities.pet", "Pet::addSpell: Non-existed in SpellStore spell #%u request.", spellId);
