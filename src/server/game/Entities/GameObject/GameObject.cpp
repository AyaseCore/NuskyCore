...
void GameObject::RemoveFromOwner()
{
    ...
    NC_LOG_FATAL("misc", "Removed GameObject (GUID: %u Entry: %u SpellId: %u LinkedGO: %u) that just lost any reference to the owner (GUID: %u Type: '%s') GO list",
...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state, uint32 artKit)
{
    ...
    if (!IsPositionValid())
    {
        NC_LOG_ERROR("misc", "Gameobject (GUID: %u Entry: %u) not created. Suggested coordinates isn't valid (X: %f Y: %f)", guidlow, name_id, x, y);
        return false;
    }
    ...
    if (!goinfo)
    {
        NC_LOG_ERROR("sql.sql", "Gameobject (GUID: %u Entry: %u) not created: non-existing entry in `gameobject_template`. Map: %u (X: %f Y: %f Z: %f)", guidlow, name_id, map->GetId(), x, y, z);
        return false;
    }
    ...
    {
        NC_LOG_ERROR("sql.sql", "Gameobject (GUID: %u Entry: %u) not created: non-existing GO type '%u' in `gameobject_template`. It will crash client if created.", guidlow, name_id, goinfo->type);
        return false;
    }
...
void GameObject::Update(uint32 diff)
{
    if (AI())
        ...
        NC_LOG_ERROR("misc", "Could not initialize GameObjectAI");
        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    {
                        Nusky::AnyUnfriendlyNoTotemUnitInObjectRangeCheck checker(this, owner, radius);
                        Nusky::UnitSearcher<Nusky::AnyUnfriendlyNoTotemUnitInObjectRangeCheck> searcher(this, ok, checker);
                    ...
                    else                                        // environmental trap
                    {
                        ...
                        // affect only players
                        Player* player = NULL;
                        Nusky::AnyPlayerInObjectRangeCheck checker(this, radius);
                        Nusky::PlayerSearcher<Nusky::AnyPlayerInObjectRangeCheck> searcher(this, player, checker);
...
void GameObject::SaveToDB()
{
    ...
    if (!data)
    {
        NC_LOG_ERROR("misc", "GameObject::SaveToDB failed, cannot get gameobject data!");
        return;
    }
...
bool GameObject::LoadGameObjectFromDB(uint32 guid, Map* map, bool addToMap)
{
    ...
    if (!data)
    {
        NC_LOG_ERROR("sql.sql", "Gameobject (GUID: %u) not found in table `gameobject`, can't load. ", guid);
        return false;
    }
...
void GameObject::TriggeringLinkedGameObject(uint32 trapEntry, Unit* target)
{
    ...
    GameObject* trapGO = NULL;
    {
        ...
        Nusky::NearestGameObjectEntryInObjectRangeCheck go_check(*target, trapEntry, range);
        Nusky::GameObjectLastSearcher<Nusky::NearestGameObjectEntryInObjectRangeCheck> checker(this, trapGO, go_check);

        TypeContainerVisitor<Nusky::GameObjectLastSearcher<Nusky::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer > object_checker(checker);
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    CellCoord p(Nusky::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    Nusky::NearestGameObjectFishingHole u_check(*this, range);
    Nusky::GameObjectSearcher<Nusky::NearestGameObjectFishingHole> checker(this, ok, u_check);

    TypeContainerVisitor<Nusky::GameObjectSearcher<Nusky::NearestGameObjectFishingHole>, GridTypeMapContainer > grid_object_checker(checker);
...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
        {
            ...
            if (user->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                {
                    NC_LOG_DEBUG("maps.script", "Goober ScriptStart id %u for GO entry %u (GUID %u).", info->goober.eventId, GetEntry(), GetDBTableGUIDLow());
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:                   //17
        {
            ...
            switch (getLootState())
            {
                case GO_READY:                              // ready for loot
                {
                    ...
                    if (!zone_skill)
                        NC_LOG_ERROR("sql.sql", "Fishable areaId %u are not properly defined in `skill_fishing_base_level`.", subzone);
                    ...
                    NC_LOG_DEBUG("misc", "Fishing check (skill: %i zone min skill: %i chance %i roll: %i", skill, zone_skill, chance, roll);
            ...
            if (GetUniqueUseCount() == info->summoningRitual.reqParticipants)
            {
                ...
                if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                    for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                        // m_unique_users can contain only player GUIDs
                        if (Player* target = ObjectAccessor::GetPlayer(*this, Nusky::Containers::SelectRandomContainerElement(m_unique_users)))
        ...
        default:
            if (GetGoType() >= MAX_GAMEOBJECT_TYPE)
                NC_LOG_ERROR("misc", "GameObject::Use(): unit (type: %u, guid: %u, name: %s) tries to use object (guid: %u, entry: %u, name: %s) of unknown type (%u)",
    ...
    if (!spellInfo)
    {
        if (user->GetTypeId() != TYPEID_PLAYER || !sOutdoorPvPMgr->HandleCustomSpell(user->ToPlayer(), spellId, this))
            NC_LOG_ERROR("misc", "WORLD: unknown spell id %u at use action for gameobject (Entry: %u GoType: %u)", spellId, GetEntry(), GetGoType());
        else
            NC_LOG_DEBUG("outdoorpvp", "WORLD: %u non-dbc spell was handled by OutdoorPvP", spellId);
        return;
    }
