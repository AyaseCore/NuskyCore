...
template<class T> T* ObjectAccessor::GetObjectInWorld(uint32 mapid, float x, float y, uint64 guid, T* /*fake*/)
{
    ...
    if (!p.IsCoordValid())
    {
        NC_LOG_ERROR("misc", "ObjectAccessor::GetObjectInWorld: invalid coordinates supplied X:%f Y:%f grid cell [%u:%u]", x, y, p.x_coord, p.y_coord);
        return NULL;
    }

    CellCoord q = Nusky::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!q.IsCoordValid())
    {
        NC_LOG_ERROR("misc", "ObjectAccessor::GetObjecInWorld: object (GUID: %u TypeId: %u) has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUIDLow(), obj->GetTypeId(), obj->GetPositionX(), obj->GetPositionY(), q.x_coord, q.y_coord);
        return NULL;
    }
...
void ObjectAccessor::RemoveCorpse(Corpse* corpse)
{
    ...
    // Critical section
    {
        ...
        CellCoord cellCoord = Nusky::ComputeCellCoord(corpse->GetPositionX(), corpse->GetPositionY());
...
Corpse* ObjectAccessor::ConvertCorpseForPlayer(uint64 player_guid, bool insignia /*=false*/)
{
    ...
    NC_LOG_DEBUG("misc", "Deleting Corpse and spawned bones.");
