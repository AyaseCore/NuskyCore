...
namespace
{
    typedef UNORDERED_MAP<uint32, Nusky::AutoPtr<Weather, ACE_Null_Mutex> > WeatherMap;
...
void LoadWeatherData()
{
    ...
    if (!result)
    {
        NC_LOG_ERROR("server.loading", ">> Loaded 0 weather definitions. DB table `game_weather` is empty.");
        return;
    }
    ...
    do
    {
        ...
        for (uint8 season = 0; season < WEATHER_SEASONS; ++season)
        {
            ...
            if (wzc.data[season].rainChance > 100)
            {
                ...
                NC_LOG_ERROR("sql.sql", "Weather for zone %u season %u has wrong rain chance > 100%%", zone_id, season);
            }

            if (wzc.data[season].snowChance > 100)
            {
                wzc.data[season].snowChance = 25;
                NC_LOG_ERROR("sql.sql", "Weather for zone %u season %u has wrong snow chance > 100%%", zone_id, season);
            }

            if (wzc.data[season].stormChance > 100)
            {
                wzc.data[season].stormChance = 25;
                NC_LOG_ERROR("sql.sql", "Weather for zone %u season %u has wrong storm chance > 100%%", zone_id, season);
            }
        }
    ...
    NC_LOG_INFO("server.loading", ">> Loaded %u weather definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
