...
int WorldSocket::SendPacket(WorldPacket const& pct)
{
   ...
   /* WorldPacket buff;
    ...
    if (m_Session)
        NC_LOG_TRACE("network.opcode", "S->C: %s %s", m_Session->GetPlayerInfo().c_str(), GetOpcodeNameForLogging(pkt->GetOpcode(), true).c_str());
    ...
    else
    {
        ...
        {
            NC_LOG_ERROR("network", "WorldSocket::SendPacket enqueue_tail failed");
            mb->release();
            return -1;
        }
    }
...
int WorldSocket::open (void *a)
{
    ...
    {
        NC_LOG_ERROR("network", "WorldSocket::open: peer().get_remote_addr errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
    ...
    {
        NC_LOG_ERROR("network", "WorldSocket::open: unable to register client handler errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
...
int WorldSocket::handle_input (ACE_HANDLE)
{
    ...
    switch (handle_input_missing_data())
    {
        case -1 :
        {
            ...
            NC_LOG_DEBUG("network", "WorldSocket::handle_input: Peer error closing connection errno = %s", ACE_OS::strerror (errno));
        ...
        case 0:
        {
            NC_LOG_DEBUG("network", "WorldSocket::handle_input: Peer has closed connection");
...
int WorldSocket::handle_output_queue (GuardType& g)
{
    ...
    {
        NC_LOG_ERROR("network", "WorldSocket::handle_output_queue dequeue_head");
        return -1;
    }
    ...
    else if (n < (ssize_t)send_len) //now n > 0
    {
        ...
        {
            NC_LOG_ERROR("network", "WorldSocket::handle_output_queue enqueue_head");
            mblk->release();
            return -1;
        }
...
int WorldSocket::handle_input_header (void)
{
    ...
    if (m_Crypt.IsInitialized())
    {
        ...
        if (header.size > 10236)
        {
            ...
            NC_LOG_ERROR("network", "WorldSocket::handle_input_header(): client (account: %u, char [GUID: %u, name: %s]) sent malformed packet (size: %d, cmd: %d)",
...
int WorldSocket::handle_input_missing_data (void)
{
    ...
    while (message_block.length() > 0)
    {
        ...
        if (!m_RecvWPct)
        {
            NC_LOG_ERROR("network", "Forcing close on input m_RecvWPct = NULL");
            errno = EINVAL;
            return -1;
        }
...
int WorldSocket::cancel_wakeup_output (GuardType& g)
{
        ...
        (this, ACE_Event_Handler::WRITE_MASK) == -1)
    {
        ...
        NC_LOG_ERROR("network", "WorldSocket::cancel_wakeup_output");
        return -1;
    }
...
int WorldSocket::schedule_wakeup_output (GuardType& g)
{
    ...
    if (reactor()->schedule_wakeup
        (this, ACE_Event_Handler::WRITE_MASK) == -1)
    {
        NC_LOG_ERROR("network", "WorldSocket::schedule_wakeup_output");
        return -1;
    }
...
int WorldSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    if (m_Session)
        NC_LOG_TRACE("network.opcode", "C->S: %s %s", m_Session->GetPlayerInfo().c_str(), opcodeName.c_str());
    ...
    try
    {
        switch (opcode)
        {
            ...
            case CMSG_AUTH_SESSION:
                if (m_Session)
                {
                    NC_LOG_ERROR("network", "WorldSocket::ProcessIncoming: received duplicate CMSG_AUTH_SESSION from %s", m_Session->GetPlayerInfo().c_str());
                    return -1;
                }
            ...
            {
                NC_LOG_DEBUG("network", "%s", opcodeName.c_str());
                sScriptMgr->OnPacketReceive(this, WorldPacket(*new_pct));
            ...
            default:
            {
                ...
                if (!m_Session)
                {
                    NC_LOG_ERROR("network.opcode", "ProcessIncoming: Client not authed opcode = %u", uint32(opcode));
                    return -1;
                }
                ...
                {
                    NC_LOG_ERROR("network.opcode", "No defined handler for opcode %s sent by %s", GetOpcodeNameForLogging(new_pct->GetOpcode(), false, new_pct->GetReceivedOpcode()).c_str(), m_Session->GetPlayerInfo().c_str());
                    return 0;
                }
    ...
    {
        NC_LOG_ERROR("network", "WorldSocket::ProcessIncoming ByteBufferException occured while parsing an instant handled packet %s from client %s, accountid=%i. Disconnected client.",
...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    if (sWorld->IsClosed())
    {
        ...
        NC_LOG_ERROR("network", "WorldSocket::HandleAuthSession: World closed, denying client (%s).", GetRemoteAddress().c_str());
        return -1;
    }
    ...
    if (!result)
    {
        ...
        NC_LOG_ERROR("network", "WorldSocket::HandleAuthSession: Sent Auth Response (unknown account).");
        return -1;
    }
    ...
    if (fields[3].GetUInt8() == 1) // if ip is locked
    {
        if (strcmp (fields[2].GetCString(), GetRemoteAddress().c_str()))
        {
            ...
            NC_LOG_DEBUG("network", "WorldSocket::HandleAuthSession: Sent Auth Response (Account IP differs).");
            return -1;
        }
    }
    ...
    {
        SendAuthResponseError(AUTH_REJECT);
        NC_LOG_ERROR("network", "WorldSocket::HandleAuthSession: Client %s attempted to log in using invalid client OS (%s).", GetRemoteAddress().c_str(), os.c_str());
        return -1;
    }
    ...
    if (banresult) // if account banned
    {
        ...
        NC_LOG_ERROR("network", "WorldSocket::HandleAuthSession: Sent Auth Response (Account banned).");
        return -1;
    }

    // Check locked state for server
    AccountTypes allowedAccountType = sWorld->GetPlayerSecurityLimit();
    NC_LOG_DEBUG("network", "Allowed Level: %u Player Level %u", allowedAccountType, AccountTypes(security));
    if (allowedAccountType > SEC_PLAYER && AccountTypes(security) < allowedAccountType)
    {
        SendAuthResponseError(AUTH_UNAVAILABLE);
        NC_LOG_INFO("network", "WorldSocket::HandleAuthSession: User tries to login but his security level is not enough");
        return -1;
    }
    ...
    {
        SendAuthResponseError(AUTH_FAILED);
        NC_LOG_ERROR("network", "WorldSocket::HandleAuthSession: Authentication failed for account: %u ('%s') address: %s", id, account.c_str(), address.c_str());
        return -1;
    }

    NC_LOG_DEBUG("network", "WorldSocket::HandleAuthSession: Client '%s' authenticated successfully from %s.",
        account.c_str(),
        address.c_str());
...
int WorldSocket::HandlePing (WorldPacket& recvPacket)
{
    ...
    else
    {
        ...
        if (diff_time < ACE_Time_Value (27))
        {
            ...
            if (max_count && m_OverSpeedPings > max_count)
            {
                ...
                {
                    NC_LOG_ERROR("network", "WorldSocket::HandlePing: %s kicked for over-speed pings (address: %s)",
    ...
    // critical section
    {
        ...
        {
            NC_LOG_ERROR("network", "WorldSocket::HandlePing: peer sent CMSG_PING, "
