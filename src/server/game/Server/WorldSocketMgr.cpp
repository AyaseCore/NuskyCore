...
class ReactorRunnable : protected ACE_Task_Base
{
    ...
    protected:
        ...
        virtual int svc()
        {
            NC_LOG_DEBUG("misc", "Network Thread Starting");
            ...
            NC_LOG_DEBUG("misc", "Network Thread exits");
...
WorldSocketMgr::StartReactiveIO (ACE_UINT16 port, const char* address)
{
    ...
    if (num_threads <= 0)
    {
        NC_LOG_ERROR("misc", "Network.Threads is wrong in your config file");
        return -1;
    }
    ...
    NC_LOG_DEBUG("misc", "Max allowed socket connections %d", ACE::max_handles());
    ...
    if (m_SockOutUBuff <= 0)
    {
        NC_LOG_ERROR("misc", "Network.OutUBuff is wrong in your config file");
        return -1;
    }
    ...
    {
        NC_LOG_ERROR("misc", "Failed to open acceptor, check if the port is free");
        return -1;
    }
...
WorldSocketMgr::OnSocketOpen (WorldSocket* sock)
{
    ...
    if (m_SockOutKBuff >= 0)
    {
        ...
        {
            NC_LOG_ERROR("misc", "WorldSocketMgr::OnSocketOpen set_option SO_SNDBUF");
            return -1;
        }
    }
    ...
    if (m_UseNoDelay)
    {
        if (sock->peer().set_option (ACE_IPPROTO_TCP,
            ...
            sizeof (int)) == -1)
        {
            NC_LOG_ERROR("misc", "WorldSocketMgr::OnSocketOpen: peer().set_option TCP_NODELAY errno = %s", ACE_OS::strerror (errno));
            return -1;
        }
    }
