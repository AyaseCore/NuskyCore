...
void BlackMarketMgr::LoadBlackMarketTemplates()
{
    ...
    if (PreparedQueryResult result = WorldDatabase.Query(WorldDatabase.GetPreparedStatement(WORLD_SEL_BLACKMARKET_TEMPLATE)))
    {
        do
        {
            ...
            {
                NC_LOG_ERROR("sql.sql", "Table `blackmarket_template` (MarketId: %u) have data for not existing creature template (Entry: %u), ignoring", blackmarket_template->MarketId, blackmarket_template->SellerNPCEntry); 
                continue; 
            }
            blackmarket_template->ItemEntry = fields[3].GetUInt32();
            if (!sObjectMgr->GetItemTemplate(blackmarket_template->ItemEntry))
            { 
                NC_LOG_ERROR("sql.sql", "Table `blackmarket_template` (MarketId: %u) have data for not existing item template (Entry: %u), ignoring.", blackmarket_template->MarketId, blackmarket_template->ItemEntry); 
                continue; 
            }
            blackmarket_template->Quantity = fields[4].GetUInt32();
            if (!blackmarket_template->Quantity)
            { 
                NC_LOG_ERROR("sql.sql", "Table `blackmarket_template` (MarketId: %u) have amount == 0 for (ItemEntry : %u) in `blackmarket_template` table, ignoring.", blackmarket_template->MarketId, blackmarket_template->ItemEntry); 
                continue; 
            } 
    ...
    NC_LOG_INFO("server.loading", ">> Loaded %u BlackMarket templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
void BlackMarketMgr::LoadBlackMarketAuctions()
{
    ...
    NC_LOG_INFO("server.loading", ">> Loaded %u BlackMarket Auctions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void BlackMarketMgr::BuildBlackMarketRequestItemsResult(WorldPacket& data, uint32 guidLow)
{
    ...
    NC_LOG_DEBUG("network", ">> Sent %u Black Market Auctions", count);
}
...
BlackMarketAuctionTemplate* BlackMarketMgr::GetTemplate(uint32 templateId) const
{
    ...
    NC_LOG_DEBUG("blackMarket", "BlackMarketMgr::GetTemplate: [%u] not found!", templateId);
    return NULL;
}
...
BlackMarketAuction* BlackMarketMgr::GetAuction(uint32 auctionId) const
{
    ...
    NC_LOG_DEBUG("blackMarket", "BlackMarketMgr::GetAuction: [%u] not found!", auctionId);
    return NULL;
}
