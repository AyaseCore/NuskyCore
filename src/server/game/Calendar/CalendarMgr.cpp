...
void CalendarMgr::LoadFromDB()
{
    ...
    NC_LOG_INFO("server.loading", ">> Loaded %u calendar events", count);
    count = 0;
    ...
    NC_LOG_INFO("server.loading", ">> Loaded %u calendar invites", count);
...
CalendarEvent* CalendarMgr::GetEvent(uint64 eventId) const
{
    ...
    NC_LOG_DEBUG("calendar", "CalendarMgr::GetEvent: [" UI64FMTD "] not found!", eventId);
    return NULL;
}
...
CalendarInvite* CalendarMgr::GetInvite(uint64 inviteId) const
{
    ...
    NC_LOG_DEBUG("calendar", "CalendarMgr::GetInvite: [" UI64FMTD "] not found!", inviteId);
    return NULL;
}
