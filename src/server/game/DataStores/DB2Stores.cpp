...
{
    NC_LOG_ERROR("misc", "Size of '%s' setted by format string (%u) not equal size of C++ structure (%u).", filename.c_str(), fsize, rsize);
...
void LoadDB2Stores(std::string const& dataPath)
{
    ...
    NC_LOG_INFO("misc", ">> Initialized %d DB2 data stores.", DB2FilesCount);
}
