...
WayPoint* SmartAI::GetNextWayPoint()
{
    ...
    if (itr != mWayPoints->end())
    {
        ...
        {
            NC_LOG_ERROR("misc", "SmartAI::GetNextWayPoint: Got not expected waypoint id %u, expected %u", mLastWP->id, mCurrentWPID);
        }
        return (*itr).second;
    }
    return NULL;
...
void SmartAI::StartPath(bool run, uint32 path, bool repeat, Unit* /*invoker*/)
{
    ...
    {
        NC_LOG_ERROR("misc", "SmartAI::StartPath: Creature entry %u wanted to start waypoint movement while in combat, ignoring.", me->GetEntry());
        return;
    }
...
void SmartAI::PausePath(uint32 delay, bool forced)
{
    ...
    {
        NC_LOG_ERROR("misc", "SmartAI::StartPath: Creature entry %u wanted to pause waypoint movement while already paused, ignoring.", me->GetEntry());
        return;
    }
...
{
    NC_LOG_DEBUG("scripts.ai", "SmartGameObjectAI::GossipHello");
...
class SmartTrigger : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* trigger)
        {
            ...
            NC_LOG_DEBUG("scripts.ai", "AreaTrigger %u is using SmartTrigger script", trigger->id);
            SmartScript script;
