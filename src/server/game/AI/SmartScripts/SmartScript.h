...
#ifndef Nusky_SMARTSCRIPT_H
#define Nusky_SMARTSCRIPT_H
...
class SmartScript
{
    public:
        ...
        bool IsSmart(Creature* c = NULL)
        {
            ...
            if (!smart)
                NC_LOG_ERROR("sql.sql", "SmartScript: Action target Creature (GUID: %u Entry: %u) is not using SmartAI, action skipped to prevent crash.", c ? c->GetDBTableGUIDLow() : (me ? me->GetDBTableGUIDLow() : 0), c ? c->GetEntry() : (me ? me->GetEntry() : 0));
        ...
        bool IsSmartGO(GameObject* g = NULL)
        {
            ...
            if (!go || go->GetAIName() != "SmartGameObjectAI")
                smart = false;
            if (!smart)
                NC_LOG_ERROR("sql.sql", "SmartScript: Action target GameObject (GUID: %u Entry: %u) is not using SmartGameObjectAI, action skipped to prevent crash.", g ? g->GetDBTableGUIDLow() : (go ? go->GetDBTableGUIDLow() : 0), g ? g->GetEntry() : (go ? go->GetEntry() : 0));
        ...
        Creature* FindCreatureNear(WorldObject* searchObject, uint32 guid) const
        {
            ...
            Nusky::CreatureWithDbGUIDCheck target_check(searchObject, guid);
            Nusky::CreatureSearcher<Nusky::CreatureWithDbGUIDCheck> checker(searchObject, creature, target_check);
