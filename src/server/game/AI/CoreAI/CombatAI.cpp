...
{
    if (!me->m_spells[0])
        NC_LOG_ERROR("misc", "ArcherAI set for creature (entry = %u) with spell1=0. AI will do nothing", me->GetEntry());
...
{
    if (!me->m_spells[0])
        NC_LOG_ERROR("misc", "TurretAI set for creature (entry = %u) with spell1=0. AI will do nothing", me->GetEntry());
...
void VehicleAI::LoadConditions()
{
    ...
    if (!conditions.empty())
        NC_LOG_DEBUG("condition", "VehicleAI::LoadConditions: loaded %u conditions", uint32(conditions.size()));
}
