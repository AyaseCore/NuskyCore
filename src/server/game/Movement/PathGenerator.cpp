...
PathGenerator::PathGenerator(const Unit* owner) :
    ...
    _navMeshQuery(NULL)
{
    NC_LOG_DEBUG("maps", "++ PathGenerator::PathGenerator for %u \n", _sourceUnit->GetGUIDLow());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest)
{
    ...
    if (!Nusky::IsValidMapCoord(destX, destY, destZ) || !Nusky::IsValidMapCoord(x, y, z))
        return false;
    ...
    NC_LOG_DEBUG("maps", "++ PathGenerator::CalculatePath() for %u \n", _sourceUnit->GetGUIDLow());
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    {
        NC_LOG_DEBUG("maps", "++ BuildPolyPath :: (startPoly == 0 || endPoly == 0)\n");
        BuildShortcut();
    ...
    if (farFromPoly)
    {
        NC_LOG_DEBUG("maps", "++ BuildPolyPath :: farFromPoly distToStartPoly=%.3f distToEndPoly=%.3f\n", distToStartPoly, distToEndPoly);
        ...
        if (_sourceUnit->GetTypeId() == TYPEID_UNIT)
        {
            ...
            {
                NC_LOG_DEBUG("maps", "++ BuildPolyPath :: underWater case\n");
                if (owner->CanSwim())
                    buildShotrcut = true;
            }
            else
            {
                NC_LOG_DEBUG("maps", "++ BuildPolyPath :: flying case\n");
                if (owner->CanFly())
                    buildShotrcut = true;
            }
        }
    ...
    {
        NC_LOG_DEBUG("maps", "++ BuildPolyPath :: (startPoly == endPoly)\n");
        ...
        NC_LOG_DEBUG("maps", "++ BuildPolyPath :: path type %d\n", _type);
        return;
    }
    ...
    {
        NC_LOG_DEBUG("maps", "++ BuildPolyPath :: (startPolyFound && endPolyFound)\n");
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult))
        {
            ...
            NC_LOG_ERROR("maps", "%u's Path Build failed: 0 length path", _sourceUnit->GetGUIDLow());
        }

        NC_LOG_DEBUG("maps", "++  m_polyLength=%u prefixPolyLength=%u suffixPolyLength=%u \n", _polyLength, prefixPolyLength, suffixPolyLength);
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    if (pointCount < 2 || dtStatusFailed(dtResult))
    {
        ...
        NC_LOG_DEBUG("maps", "++ PathGenerator::BuildPointPath FAILED! path sized %d returned\n", pointCount);
        BuildShortcut();
        _type = PATHFIND_NOPATH;
        return;
    }
    ...
    {
        NC_LOG_DEBUG("maps", "++ PathGenerator::BuildPointPath FAILED! path sized %d returned, lower than limit set to %d\n", pointCount, _pointPathLimit);
        BuildShortcut();
    ...
    NC_LOG_DEBUG("maps", "++ PathGenerator::BuildPointPath path type %d size %d poly-size %d\n", _type, pointCount, _polyLength);
}
...
{
    NC_LOG_DEBUG("maps", "++ BuildShortcut :: making shortcut\n");
