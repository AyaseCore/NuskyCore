...
void MoveSpline::init_spline(const MoveSplineInitArgs& args)
{
    ...
    {
        NC_LOG_ERROR("misc", "MoveSpline::init_spline: zero length spline, wrong input data?");
...
#define CHECK(exp) \
    if (!(exp))\
    {\
        NC_LOG_ERROR("misc", "MoveSplineInitArgs::Validate: expression '%s' failed for GUID: %u Entry: %u", #exp, unit->GetTypeId() == TYPEID_PLAYER ? unit->GetGUIDLow() : unit->ToCreature()->GetDBTableGUIDLow(), unit->GetEntry());\
        return false;\
    }
    CHECK(path.size() > 1);
    CHECK(velocity > 0.1f);
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Catmullrom) && path.size() > 2)
    {
        ...
        for (uint32 i = 1; i < path.size()-1; ++i)
        {
            ...
            {
                NC_LOG_ERROR("misc", "MoveSplineInitArgs::_checkPathBounds check failed");
                return false;
            }
        }
    }
