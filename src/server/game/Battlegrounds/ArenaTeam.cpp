                                         ...
                                         uint8 borderStyle, uint32 borderColor)
{
    ...
    NC_LOG_DEBUG("bg.arena", "New ArenaTeam created [Id: %u, Name: %s] [Type: %u] [Captain low GUID: %u]", GetId(), GetName().c_str(), GetType(), captainLowGuid);
    return true;
}
...
bool ArenaTeam::AddMember(uint64 playerGuid)
{
    ...
    {
        NC_LOG_DEBUG("bg.arena", "Arena: Player %s (guid: %u) already has an arena team of type %u", playerName.c_str(), GUID_LOPART(playerGuid), GetType());
        return false;
    }
    ...
    NC_LOG_DEBUG("bg.arena", "Player: %s [GUID: %u] joined arena team type: %u [Id: %u, Name: %s].", playerName.c_str(), GUID_LOPART(playerGuid), GetType(), GetId(), GetName().c_str());
...
bool ArenaTeam::LoadMembersFromDB(QueryResult result)
{
    ...
    do
    {
        ...
        {
            NC_LOG_ERROR("sql.sql", "ArenaTeam %u has member with empty name - probably player %u doesn't exist, deleting him from memberlist!", arenaTeamId, GUID_LOPART(newMember.Guid));
    ...
    if (Empty() || !captainPresentInTeam)
    {
        ...
        NC_LOG_DEBUG("bg.arena", "ArenaTeam %u does not have any members or its captain is not in team, disbanding it...", TeamId);
        return false;
    }
...
void ArenaTeam::SetCaptain(uint64 guid)
{
    ...
    if (Player* newCaptain = ObjectAccessor::FindPlayer(guid))
    {
        ...
        if (oldCaptain)
        {
            NC_LOG_DEBUG("bg.arena", "Player: %s [GUID: %u] promoted player: %s [GUID: %u] to leader of arena team [Id: %u, Name: %s] [Type: %u].",
...
void ArenaTeam::DelMember(uint64 guid, bool cleanDb)
{
    ...
    if (Player* player = ObjectAccessor::FindPlayer(guid))
    {
        ...
        NC_LOG_DEBUG("bg.arena", "Player: %s [GUID: %u] left arena team type: %u [Id: %u, Name: %s].", player->GetName().c_str(), player->GetGUIDLow(), GetType(), GetId(), GetName().c_str());
    }
...
void ArenaTeam::Disband(WorldSession* session)
{
    ...
    if (session)
    {
        ...
        if (Player* player = session->GetPlayer())
            NC_LOG_DEBUG("bg.arena", "Player: %s [GUID: %u] disbanded arena team type: %u [Id: %u, Name: %s].", player->GetName().c_str(), player->GetGUIDLow(), GetType(), GetId(), GetName().c_str());
    }
...
void ArenaTeam::Roster(WorldSession* session)
{
    ...
    NC_LOG_DEBUG("network", "WORLD: Sent SMSG_ARENA_TEAM_ROSTER");
}
...
void ArenaTeam::Query(WorldSession* session)
{
    ...
    session->SendPacket(&data);
    NC_LOG_DEBUG("network", "WORLD: Sent SMSG_ARENA_TEAM_QUERY_RESPONSE");
}
...
void ArenaTeam::BroadcastEvent(ArenaTeamEvents event, uint64 guid, uint8 strCount, std::string const& str1, std::string const& str2, std::string const& str3)
{
    ...
    switch (strCount)
    {
        ...
        case 3:
            ...
            break;
        default:
            NC_LOG_ERROR("bg.arena", "Unhandled strCount %u in ArenaTeam::BroadcastEvent", strCount);
            return;
    }
    ...
    NC_LOG_DEBUG("network", "WORLD: Sent SMSG_ARENA_TEAM_EVENT");
}
...
uint8 ArenaTeam::GetSlotByType(uint32 type)
{
    ...
    NC_LOG_ERROR("bg.arena", "FATAL: Unknown arena team type %u for some arena team", type);
    return 0xFF;
}
...
uint8 ArenaTeam::GetTypeBySlot(uint8 slot)
{
    ...
    NC_LOG_ERROR("bg.arena", "FATAL: Unknown arena team slot %u for some arena team", slot);
    return 0xFF;
}
