...
uint32 ArenaTeamMgr::GenerateArenaTeamId()
{
    ...
    {
        NC_LOG_ERROR("bg.battleground", "Arena team ids overflow!! Can't continue, shutting down server. ");
...
void ArenaTeamMgr::LoadArenaTeams()
{
    ...
    if (!result)
    {
        NC_LOG_INFO("server.loading", ">> Loaded 0 arena teams. DB table `arena_team` is empty!");
        return;
    }
    ...
    NC_LOG_INFO("server.loading", ">> Loaded %u arena teams in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
