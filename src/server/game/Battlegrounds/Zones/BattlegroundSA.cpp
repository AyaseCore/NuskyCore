...
bool BattlegroundSA::ResetObjs()
{
    ...
    for (uint8 i = 0; i < BG_SA_MAX_GY; i++)
    {
        ...
        if (!sg)
        {
            NC_LOG_ERROR("bg.battleground", "SOTA: Can't find GY entry %u", BG_SA_GYEntries[i]);
            return false;
        }
        ...
        else
        {
            ...
            if (!AddSpiritGuide(i + BG_SA_MAXNPC, sg->x, sg->y, sg->z, BG_SA_GYOrientation[i], ((Attackers == TEAM_HORDE)? ALLIANCE : HORDE)))
                NC_LOG_ERROR("bg.battleground", "SOTA: couldn't spawn GY: %u", i);
        }
    }
...
void BattlegroundSA::CaptureGraveyard(BG_SA_Graveyards i, Player* Source)
{
    ...
    if (!sg)
    {
        NC_LOG_ERROR("bg.battleground", "BattlegroundSA::CaptureGraveyard: non-existant GY entry: %u", BG_SA_GYEntries[i]);
        return;
    }
