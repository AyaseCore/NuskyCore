...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    char buf[256];
    if (IsTower(node))
        sprintf(buf, GetNuskyString(LANG_BG_AV_TOWER_TAKEN), GetNodeName(node), (owner == ALLIANCE) ? GetNuskyString(LANG_BG_AV_ALLY) : GetNuskyString(LANG_BG_AV_HORDE));
    else
        sprintf(buf, GetNuskyString(LANG_BG_AV_GRAVE_TAKEN), GetNodeName(node), (owner == ALLIANCE) ? GetNuskyString(LANG_BG_AV_ALLY) :GetNuskyString(LANG_BG_AV_HORDE));
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, uint32 team, bool initial)
{
    ...
    if (team == ALLIANCE || team == HORDE)
    {
        ...
        char buf[256];
        sprintf(buf, GetNuskyString(LANG_BG_AV_MINE_TAKEN), GetNuskyString((mine == AV_NORTH_MINE) ? LANG_BG_AV_MINE_NORTH : LANG_BG_AV_MINE_SOUTH),
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
   ...
   //spawn new go :)
    ...
    //send a nice message to all :)
    char buf[256];
    sprintf(buf, GetNuskyString((IsTower(node)) ? LANG_BG_AV_TOWER_DEFENDED : LANG_BG_AV_GRAVE_DEFENDED), GetNodeName(node),
            (team == ALLIANCE) ?  GetNuskyString(LANG_BG_AV_ALLY) : GetNuskyString(LANG_BG_AV_HORDE));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
    if (creature)
        YellToAll(creature, buf, LANG_UNIVERSAL);
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    //send a nice message to all :)
    char buf[256];
    sprintf(buf, (IsTower(node)) ? GetNuskyString(LANG_BG_AV_TOWER_ASSAULTED) : GetNuskyString(LANG_BG_AV_GRAVE_ASSAULTED), GetNodeName(node),
            (team == ALLIANCE) ?  GetNuskyString(LANG_BG_AV_ALLY) : GetNuskyString(LANG_BG_AV_HORDE));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
    if (creature)
        YellToAll(creature, buf, LANG_UNIVERSAL);
