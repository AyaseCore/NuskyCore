...
SpellCastResult SpellInfo::CheckShapeshift(uint32 form) const
{
    ...
    if (form > 0)
    {
        ...
        if (!shapeInfo)
        {
            NC_LOG_ERROR("spells", "GetErrorAtShapeshiftedCast: unknown shapeshift %u", form);
            return SPELL_CAST_OK;
        }
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask) const
{
    ...
    if (AttributesEx & SPELL_ATTR1_DRAIN_ALL_POWER)
    {
        ...
        NC_LOG_ERROR("spells", "SpellInfo::CalcPowerCost: Unknown power type '%d' in spell %d", PowerType, Id);
        return 0;
    }
    ...
    if (ManaCostPercentage)
    {
        switch (PowerType)
        {
            ...
            case POWER_RUNIC_POWER:
                NC_LOG_DEBUG("spells", "CalculateManaCost: Not implemented yet!");
                break;
            default:
                NC_LOG_ERROR("spells", "CalculateManaCost: Unknown power type '%d' in spell %d", PowerType, Id);
                return 0;
        }
    }
