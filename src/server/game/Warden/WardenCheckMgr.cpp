...
void WardenCheckMgr::LoadWardenChecks()
{
    ...
    {
        NC_LOG_INFO("warden", ">> Warden disabled, loading checks skipped.");
        return;
    }
    ...
    if (!result)
    {
        NC_LOG_INFO("server.loading", ">> Loaded 0 Warden checks. DB table `warden_checks` is empty!");
        return;
    }
    ...
    NC_LOG_INFO("server.loading", ">> Loaded %u warden checks.", count);
}
...
void WardenCheckMgr::LoadWardenOverrides()
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_WARDEN_ENABLED))
    {
        NC_LOG_INFO("warden", ">> Warden disabled, loading check overrides skipped.");
        return;
    }
    ...
    if (!result)
    {
        NC_LOG_INFO("server.loading", ">> Loaded 0 Warden action overrides. DB table `warden_action` is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (action > WARDEN_ACTION_BAN)
            NC_LOG_ERROR("warden", "Warden check override action out of range (ID: %u, action: %u)", checkId, action);
        // Check if check actually exists before accessing the CheckStore vector
        else if (checkId > CheckStore.size())
            NC_LOG_ERROR("warden", "Warden check action override for non-existing check (ID: %u, action: %u), skipped", checkId, action);
        else
        {
    ...
    NC_LOG_INFO("server.loading", ">> Loaded %u warden action overrides.", count);
}
