...
{
    NC_LOG_DEBUG("warden", "Send module to client");
...
{
    NC_LOG_DEBUG("warden", "Request module");
    ...
    if (_initialized)
    {
        ...
        if (_dataSent)
        {
            ...
            if (maxClientResponseDelay > 0)
            {
                ...
                {
                    NC_LOG_WARN("warden", "%s (latency: %u, IP: %s) exceeded Warden module response delay for more than %s - disconnecting client",
...
bool Warden::IsValidCheckSum(uint32 checksum, const uint8* data, const uint16 length)
{
    ...
    {
        NC_LOG_DEBUG("warden", "CHECKSUM IS NOT VALID");
        return false;
    }
    else
    {
        NC_LOG_DEBUG("warden", "CHECKSUM IS VALID");
        return true;
    }
}
...
void WorldSession::HandleWardenDataOpcode(WorldPacket& recvData)
{
    ...
    uint8 opcode;
    recvData >> opcode;
    NC_LOG_DEBUG("warden", "Got packet, opcode %02X, size %u", opcode, uint32(recvData.size()));
    recvData.hexlike();
    ...
    switch (opcode)
    {
        ...
        case WARDEN_CMSG_MEM_CHECKS_RESULT:
            NC_LOG_DEBUG("warden", "NYI WARDEN_CMSG_MEM_CHECKS_RESULT received!");
            break;
        ...
        case WARDEN_CMSG_MODULE_FAILED:
            NC_LOG_DEBUG("warden", "NYI WARDEN_CMSG_MODULE_FAILED received!");
            break;
        default:
            NC_LOG_DEBUG("warden", "Got unknown warden opcode %02X of size %u.", opcode, uint32(recvData.size() - 1));
            break;
    }
}
