...
public:
    ...
    static bool HandleGMVisibleCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            handler->PSendSysMessage(LANG_YOU_ARE, handler->GetSession()->GetPlayer()->isGMVisible() ? handler->GetNuskyString(LANG_VISIBLE) : handler->GetNuskyString(LANG_INVISIBLE));
            return true;
        }
