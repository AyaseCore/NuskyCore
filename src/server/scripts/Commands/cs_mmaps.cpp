...
public:
    ...
    static bool HandleMmapTestArea(ChatHandler* handler, char const* /*args*/)
    {
        ...
        CellCoord pair(Nusky::ComputeCellCoord(object->GetPositionX(), object->GetPositionY()));
        Cell cell(pair);
        cell.SetNoCreate();

        std::list<Creature*> creatureList;

        Nusky::AnyUnitInObjectRangeCheck go_check(object, radius);
        Nusky::CreatureListSearcher<Nusky::AnyUnitInObjectRangeCheck> go_search(object, creatureList, go_check);
        TypeContainerVisitor<Nusky::CreatureListSearcher<Nusky::AnyUnitInObjectRangeCheck>, GridTypeMapContainer> go_visit(go_search);
