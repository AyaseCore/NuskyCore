...
public:
    ...
    static bool HandleDebugEnterVehicleCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            Nusky::AllCreaturesOfEntryInRange check(handler->GetSession()->GetPlayer(), entry, 20.0f);
            Nusky::CreatureSearcher<Nusky::AllCreaturesOfEntryInRange> searcher(handler->GetSession()->GetPlayer(), passenger, check);
    ...
    static bool HandleWPGPSCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        NC_LOG_INFO("sql.dev", "(@PATH, XX, %.3f, %.3f, %.5f, 0, 0, 0, 100, 0),", player->GetPositionX(), player->GetPositionY(), player->GetPositionZ());
