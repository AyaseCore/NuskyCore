                ...
                switch (action)
                {
                    ...
                    case ACTION_ROTFACE_COMBAT:
                    {
                        ...
                        if (Creature* rotface = Unit::GetCreature(*me, instance->GetData64(DATA_ROTFACE)))
                        {
                            ...
                            if (list.size() > 4)
                            {
                                list.sort(Nusky::ObjectDistanceOrderPred(rotface));
                                do
                                {
                                    list.pop_back();
                                } while (list.size() > 4);
    ...
    public:
        ...
        class spell_putricide_unbound_plague_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                targets.remove_if(Nusky::UnitAuraCheck(true, sSpellMgr->GetSpellIdForDifficulty(SPELL_UNBOUND_PLAGUE, GetCaster())));
