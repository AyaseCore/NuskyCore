    ...
    public:
        ...
        class spell_rotface_ooze_flood_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                triggers.sort(Nusky::ObjectDistanceOrderPred(GetHitUnit()));
        ...
        class spell_rotface_vile_gas_trigger_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                while (ranged.size() < minTargets)
                {
                    ...
                    WorldObject* target = Nusky::Containers::SelectRandomContainerElement(melee);
                    ranged.push_back(target);
                    melee.remove(target);
                }

                if (!ranged.empty())
                    Nusky::Containers::RandomResizeList(ranged, GetCaster()->GetMap()->Is25ManRaid() ? 3 : 1);
