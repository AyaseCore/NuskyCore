...
void hyjalAI::Reset()
{
    ...
    } else NC_LOG_ERROR("scripts", ERROR_INST_DATA);
...
void hyjalAI::SummonNextWave(const Wave wave[18], uint32 Count, float Base[4][3])
{
    ...
    if (!instance)
    {
        NC_LOG_ERROR("scripts", ERROR_INST_DATA);
        return;
    }
    ...
    if (!wave[Count].IsBoss)
    {
        ...
        {
            NextWaveTimer = 15000;
            NC_LOG_DEBUG("scripts", "HyjalAI: debug mode is enabled. Next Wave in 15 seconds");
        }
    }
    else
    {
...
{
    CellCoord pair(Nusky::ComputeCellCoord(x, y));
    Cell cell(pair);
    cell.SetNoCreate();

    // First get all creatures.
    std::list<Creature*> creatures;
    Nusky::AllFriendlyCreaturesInGrid creature_check(me);
    Nusky::CreatureListSearcher<Nusky::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);

    TypeContainerVisitor <Nusky::CreatureListSearcher<Nusky::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    Nusky::WorldObjectWorker<Nusky::RespawnDo> worker(me, u_do);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        TypeContainerVisitor
            <Nusky::CreatureListSearcher<Nusky::AllFriendlyCreaturesInGrid>,
