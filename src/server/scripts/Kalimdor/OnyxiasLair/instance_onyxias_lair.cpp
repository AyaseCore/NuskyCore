    ...
    struct instance_onyxias_lair_InstanceMapScript : public InstanceScript
    {
        ...
        void FloorEruption(uint64 floorEruptedGUID)
        {
            if (GameObject* floorEruption = instance->GetGameObject(floorEruptedGUID))
            {
                ...
                Nusky::GameObjectInRangeCheck check(floorEruption->GetPositionX(), floorEruption->GetPositionY(), floorEruption->GetPositionZ(), 15);
                Nusky::GameObjectListSearcher<Nusky::GameObjectInRangeCheck> searcher(floorEruption, nearFloorList, check);
