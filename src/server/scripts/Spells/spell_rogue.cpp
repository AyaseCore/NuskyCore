    ...
    public:
        ...
        class spell_rog_blade_flurry_AuraScript : public AuraScript
        {
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                ...
                NC_LOG_ERROR("misc", "damage: %u procSpell: %u",
                ...
                if (Unit* target = GetHitUnit())
                {
                    ...
                    for (uint8 slot = 0; slot < MAX_ENCHANTMENT_SLOT; ++slot)
                    {
                        ...
                        for (uint8 s = 0; s < 3; ++s)
                        {
                            ...
                            if (!spellInfo)
                            {
                                NC_LOG_ERROR("spells", "Player::CastItemCombatSpell Enchant %i, player (Name: %s, GUID: %u) cast unknown spell %i", enchant->ID, player->GetName().c_str(), player->GetGUIDLow(), enchant->spellid[s]);
                                continue;
                            }
