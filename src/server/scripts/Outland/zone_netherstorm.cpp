...
/// @todo clean up this workaround when Nusky adds support to do it properly (with gossip selections instead of instant summon)
...
public:
    ...
    struct npc_commander_dawnforgeAI : public ScriptedAI
    {
        ...
        bool CanStartEvent(Player* player)
        {
            ...
            NC_LOG_DEBUG("scripts", "npc_commander_dawnforge event already in progress, need to wait.");
            return false;
        }
    ...
    struct npc_phase_hunterAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) OVERRIDE
        {
            ...
            if (ManaBurnTimer <= diff)
            {
                ...
                {
                    DoCast(Nusky::Containers::SelectRandomContainerElement(UnitsWithMana), SPELL_MANA_BURN);
