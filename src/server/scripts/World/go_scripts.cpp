...
public:
    ...
    /// @todo use gossip option ("Transcript the Tablet") instead, if Nusky adds support.
    bool OnGossipHello(Player* player, GameObject* go) OVERRIDE
    {
            ...
            if (!creature->IsHostileTo(player))
            {
                if (FactionTemplateEntry const* pFaction = creature->GetFactionTemplateEntry())
                {
                    ...
                    if (Spell)
                        ...
                        NC_LOG_ERROR("scripts", "go_ethereum_prison summoned Creature (entry %u) but faction (%u) are not expected by script.", creature->GetEntry(), creature->getFaction());
                }
            }
        }
