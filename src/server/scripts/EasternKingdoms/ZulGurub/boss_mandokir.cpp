            ...
            void DoAction(int32 action) OVERRIDE
            {
                switch (action)
                {
                    ...
                    case ACTION_START_REVIVE:
                    {
                        ...
                        creatures.remove_if(Nusky::AnyDeadUnitCheck());
                        creatures.remove_if(Nusky::UnitAuraCheck(true, SPELL_OHGAN_ORDERS_TRIGGER));
                        Nusky::Containers::RandomResizeList(creatures, 1);
                        if (creatures.empty())
                            return;
