        ...
        boss_selin_fireheartAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            if (instance)
            {
                ...
                for (uint8 i = 0; i < size; ++i)
                {
                    ...
                    NC_LOG_DEBUG("scripts", "Selin: Adding Fel Crystal " UI64FMTD " to list", guid);
        ...
        void MovementInform(uint32 type, uint32 id) OVERRIDE
        {
            if (type == POINT_MOTION_TYPE && id == 1)
            {
                ...
                else
                {
                    ...
                    NC_LOG_ERROR("scripts", "Selin Fireheart unable to drain crystal as the crystal is either dead or despawned");
                    DrainingCrystal = false;
                }
            }
        }
